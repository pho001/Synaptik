...
class NativeCpuPartitionSelectionTest {
    @Test
    void preparedExecutionNoLongerEmitsNativeCpuChainOrPartitionTraceAttrs() {
        ...
        assertFalse(trace.steps().stream()
                .flatMap(step -> step.metadata().attributes().keySet().stream())
                .anyMatch(key -> key.startsWith("nativeCpuPartition")));
    }
}
