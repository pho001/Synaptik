...
final class AbcBf16NonCheapStridedWidthBenchmarkTest {
    private static final tuning.measure.MeasurementPolicy MEASUREMENT = DebugMeasurementPolicies.STANDARD_WITH_TRACE;

    @Test
    void compareCurrentBestAgainstNonCheapStridedWidths() {
        ExecutionProfile best = loadBestProfile();
        ExecutionProfile width2 = withRuntime(best, "noncheap-strided-w2", runtimeWithNonCheapStridedAsmWidth(best.runtime(), 2));
        ExecutionProfile width4 = withRuntime(best, "noncheap-strided-w4", runtimeWithNonCheapStridedAsmWidth(best.runtime(), 4));

        BenchmarkRequest request = new BenchmarkRequest(
                StandardWorkloads.abcSequenceMatmulBlasBenchmark("abc_sequence_matmul_bf16_noncheap_strided_probe"),
                List.of(
                        BenchmarkEntry.candidate("best-current", best),
                        BenchmarkEntry.candidate("best-current-noncheap-strided-w2", width2),
                        BenchmarkEntry.candidate("best-current-noncheap-strided-w4", width4)
                ),
                MEASUREMENT,
        ...
        System.out.println();
        System.out.println("ABC_BF16_NONCHEAP_STRIDED_WIDTH_BENCHMARK");
