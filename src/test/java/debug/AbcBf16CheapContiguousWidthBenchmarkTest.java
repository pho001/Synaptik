...
final class AbcBf16CheapContiguousWidthBenchmarkTest {
    private static final tuning.measure.MeasurementPolicy MEASUREMENT = DebugMeasurementPolicies.STANDARD_WITH_TRACE;

    @Test
    void compareCurrentBestAgainstCheapContiguousWidths() {
        ExecutionProfile best = loadBestProfile();
        ExecutionProfile width2 = withRuntime(best, "cheap-contiguous-w2", runtimeWithCheapContiguousAsmWidth(best.runtime(), 2));
        ExecutionProfile width4 = withRuntime(best, "cheap-contiguous-w4", runtimeWithCheapContiguousAsmWidth(best.runtime(), 4));

        BenchmarkRequest request = new BenchmarkRequest(
                StandardWorkloads.abcSequenceMatmulBlasBenchmark("abc_sequence_matmul_bf16_cheap_contiguous_probe"),
                List.of(
                        BenchmarkEntry.candidate("best-current", best),
                        BenchmarkEntry.candidate("best-current-cheap-contiguous-w2", width2),
                        BenchmarkEntry.candidate("best-current-cheap-contiguous-w4", width4)
                ),
                MEASUREMENT,
        ...
        System.out.println();
        System.out.println("ABC_BF16_CHEAP_CONTIGUOUS_WIDTH_BENCHMARK");
    ...
    private static RuntimeConfig runtimeWithCheapContiguousAsmWidth(RuntimeConfig base, int width) {
