...
public class CpuSigmoidKernel implements CpuKernel {
    ...
    public void forward(Operation op, List<Tensor> inputs, Tensor node, CpuExecutionConfig config) {
        ...
        switch (mode) {
            case VECTOR, SCALAR -> scalarSigmoid(in, out, 0, out.length);
            case PARALLEL, PARALLEL_VECTOR -> parallelSigmoid(in, out, config);
        }
    }

    private static void parallelSigmoid(double[] in, double[] out, CpuExecutionConfig config) {
        ...
        CpuThreadPool.runChunks(chunks, config.plannedWorkers(), chunk -> {
            ...
            scalarSigmoid(in, out, start, end);
        });
    }

    private static void scalarSigmoid(double[] in, double[] out, int start, int end) {
