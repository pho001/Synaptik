...
final class CrossEntropyLossIndicesGradExecutor {
    ...
    private static void computeGroupBF16(
        ...
        float sampleScale = TensorDTypeOps.fromBFloat16Bits(scale[scaleOffset]);
        if (sampleScale == 0.0f) {
        ...
        float max = Float.NEGATIVE_INFINITY;
        for (int i = 0, logitsOffset = baseLogits; i < axisSize; i++, logitsOffset += axisStride) {
            max = Math.max(max, TensorDTypeOps.fromBFloat16Bits(logits[logitsOffset]));
        }
        float sum = 0.0f;
        for (int i = 0, logitsOffset = baseLogits, outputOffset = outOffset; i < axisSize; i++, logitsOffset += axisStride, outputOffset += outAxisStride) {
            float value = (float) Math.exp(TensorDTypeOps.fromBFloat16Bits(logits[logitsOffset]) - max);
            out[outputOffset] = TensorDTypeOps.toBFloat16Bits(value);
            sum += value;
        }
        float inv = sampleScale / sum;
        for (int i = 0, outputOffset = outOffset; i < axisSize; i++, outputOffset += outAxisStride) {
            float value = TensorDTypeOps.fromBFloat16Bits(out[outputOffset]) * inv;
            if (i == targetIndex) {
                value -= sampleScale;
            }
            out[outputOffset] = TensorDTypeOps.toBFloat16Bits(value);
