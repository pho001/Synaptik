...
final class ReductionMinMaxGradExecutor {
    private ReductionMinMaxGradExecutor() {}
    ...
    private static void runAllF16(short[] input, int[] inputShape, int[] inputStrides, int inputBaseOffset, short reducedValueBits, short gradValueBits, short[] out, int outBaseOffset, boolean isMax) {
        float reducedValue = TensorDTypeOps.fromBFloat16Bits(reducedValueBits);
        float gradValue = TensorDTypeOps.fromBFloat16Bits(gradValueBits);
        int logicalSize = logicalSize(inputShape);
        int[] denseStrides = TensorMetadata.computeStrides(inputShape);
        int winners = 0;
        for (int logical = 0; logical < logicalSize; logical++) {
            float value = TensorDTypeOps.fromBFloat16Bits(input[logicalToOffset(logical, inputShape, inputStrides, denseStrides, inputBaseOffset)]);
            if (matches(value, reducedValue, isMax)) {
                winners++;
            }
        }
        ...
        for (int logical = 0; logical < logicalSize; logical++) {
            float value = TensorDTypeOps.fromBFloat16Bits(input[logicalToOffset(logical, inputShape, inputStrides, denseStrides, inputBaseOffset)]);
            out[outBaseOffset + logical] = TensorDTypeOps.toBFloat16Bits(matches(value, reducedValue, isMax) ? share : 0.0f);
        }
    }
    ...
    private static void runAxisF16(short[] input, int[] inputShape, int[] inputStrides, int inputBaseOffset, short[] reduced, int[] reducedShape, int reducedBaseOffset, short[] outGrad, short[] out, int outBaseOffset, int dimension, boolean isMax) {
        ...
        for (int group = 0; group < groups; group++) {
            int baseInput = reductionBaseOffset(group, reducedShape, reducedDense, inputStrides, dimension, inputBaseOffset);
            int baseOut = reductionBaseOffset(group, reducedShape, reducedDense, outDense, dimension, outBaseOffset);
            float reducedValue = TensorDTypeOps.fromBFloat16Bits(reduced[reducedBaseOffset + group]);
            int winners = 0;
            for (int r = 0; r < reducedSize; r++) {
                if (matches(TensorDTypeOps.fromBFloat16Bits(input[baseInput + r * inStep]), reducedValue, isMax)) {
                    winners++;
                }
            }
            float share = winners == 0 ? 0.0f : TensorDTypeOps.fromBFloat16Bits(outGrad[outBaseOffset + group]) / winners;
            for (int r = 0; r < reducedSize; r++) {
                float value = TensorDTypeOps.fromBFloat16Bits(input[baseInput + r * inStep]);
                out[baseOut + r * outStep] = TensorDTypeOps.toBFloat16Bits(matches(value, reducedValue, isMax) ? share : 0.0f);
            }
        }
    }
