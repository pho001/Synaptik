...
final class PreparedInputPolicy {
    ...
    static boolean bypassPreparation(Operation op) {
        ...
        return switch (op.opType()) {
            case CONTIGUOUS, RESHAPE, EXPAND, SELECT, SLICE, SLICE_GRAD, CONCAT, PAD, TILE, PERMUTE, EXPAND_DIMS, SQUEEZE, CAST,
                    GATHER_AXIS, GATHER_AXIS_GRAD, GATHER_ND, GATHER_ND_GRAD,
                    SUM, MEAN, REDUCE_MIN, REDUCE_MAX, REDUCE_PROD, ARGMAX,
    ...
    static boolean requiresPreparedInput(
        ...
        if (input.hasStorageOffset()) {
            ...
            return switch (op.opType()) {
                case RESHAPE, EXPAND, SELECT, SLICE, SLICE_GRAD, CONCAT, PAD, TILE, PERMUTE, EXPAND_DIMS, SQUEEZE, CAST,
                        GATHER, GATHER_GRAD, GATHER_AXIS, GATHER_AXIS_GRAD,
                        GATHER_ND, GATHER_ND_GRAD, TAKE_ALONG_AXIS, TAKE_ALONG_AXIS_GRAD, SCATTER_ADD, SCATTER_ELEMENTS, SCATTER_ND,
                        SUM, MEAN, REDUCE_MIN, REDUCE_MAX, REDUCE_PROD, ARGMAX, REDUCE_ALL, REDUCE_ANY,
                        SOFTMAX, SOFTMAX_GRAD, LOG_SOFTMAX, LOG_SOFTMAX_GRAD, SCALED_DOT_PRODUCT_ATTENTION_WEIGHTS,
                        NLL_LOSS, CROSS_ENTROPY_LOSS, CROSS_ENTROPY_LOSS_INDICES, CROSS_ENTROPY_LOSS_INDICES_GRAD,
