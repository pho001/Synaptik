...
public final class UnaryMemorySegmentLoops {
    private UnaryMemorySegmentLoops() {
    }
    ...
    public static void execute(UnaryElementwiseKernel kernel, List<Tensor> inputs, Tensor node, CpuKernelContext context) {
        ...
        try {
            ...
            NativeTensorStorage outputStorage = ElementwiseNativeSupport.allocateNativeOutput(node, context, "unary-memory-segment-loop-" + label);
    ...
    public static void execute(
        ...
        try {
            String label = opLabel(opType);
            NativeTensorStorage inputStorage = ElementwiseNativeSupport.requireNativeInput(context, 0, node.getDataType(), label.toUpperCase());
            NativeTensorStorage outputStorage = ElementwiseNativeSupport.allocateNativeOutput(node, context, "scalar-unary-memory-segment-loop-" + label);
            CpuStorageBindings bindings = new CpuStorageBindings(
                    List.of(ElementwiseNativeSupport.segmentView(inputs.get(0), inputStorage)),
                    ElementwiseNativeSupport.segmentView(node, outputStorage)
            );
    ...
    static void runSegmentDense(UnaryElementwiseKernel kernel, CpuStorageBindings bindings) {
        ...
        switch (dtype) {
            ...
            case INT32, INT64, BOOL -> throw new UnsupportedOperationException("Unary MemorySegment loop does not support dtype: " + dtype);
        }
    }
    ...
    static void runSegmentDense(
        ...
        switch (dtype) {
            ...
            case INT32, INT64, BOOL -> throw new UnsupportedOperationException("Scalar unary MemorySegment loop does not support dtype: " + dtype);
        }
    }
    ...
    private static void validateDenseUnary(CpuStorageBindings bindings) {
        if (bindings.inputs().size() != 1) {
            throw new IllegalArgumentException("Unary MemorySegment loop requires exactly 1 input.");
        }
        CpuStorageView input = bindings.input(0);
        CpuStorageView output = bindings.output();
        if (input.dtype() != output.dtype()) {
            throw new IllegalArgumentException("Unary MemorySegment loop dtype mismatch.");
        }
        if (input.kind() != output.kind()) {
            throw new IllegalArgumentException("Unary MemorySegment loop requires matching storage kinds.");
        }
        if (input.logicalSize() != output.logicalSize()) {
            throw new IllegalArgumentException("Unary MemorySegment loop requires same-shape dense input.");
        }
        if (!ElementwiseNativeSupport.isDenseView(input) || !ElementwiseNativeSupport.isDenseView(output)) {
            throw new IllegalArgumentException("Unary MemorySegment loop requires dense zero-offset views.");
        }
    }
    ...
    private static void attachNativeOutput(
        ...
        context.executionContext().attachNativeStorage(
                context.nodeId(),
                outputStorage,
                "unary MemorySegment loop " + label.toUpperCase() + " wrote " + node.getDataType() + " native output"
        );
    }
