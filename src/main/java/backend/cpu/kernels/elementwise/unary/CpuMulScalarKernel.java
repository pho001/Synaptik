...
public final class CpuMulScalarKernel extends StorageAwareScalarUnaryElementwiseKernel {
    ...
    protected Operation.OpType opType() {
        return Operation.OpType.MUL_SCALAR;
    }
    ...
    protected ScalarUnaryParameters extractParameters(Operation operation) {
        mulScalar mul = (mulScalar) operation;
        return new ScalarUnaryParameters(mul.getScalar(), mul.getScalarF32());
    }
    ...
    private static void runArrayBF16(short[] in, float parameter, short[] out, int start, int end) {
        for (int i = start; i < end; i++) {
            out[i] = TensorDTypeOps.toBFloat16Bits(TensorDTypeOps.fromBFloat16Bits(in[i]) * parameter);
        }
    }

    private static void runArrayBF16ToFloat(short[] in, float parameter, float[] out, int start, int end) {
        for (int i = start; i < end; i++) out[i] = TensorDTypeOps.fromBFloat16Bits(in[i]) * parameter;
    }
    ...
    protected void runIndexedArrayF64(
        ...
        for (int outIndex = start; outIndex < end; outIndex++) {
            out[cursor.offset(0)] = in[cursor.offset(1)] * parameter;
            if (outIndex + 1 < end) {
                cursor.step();
            }
        }
    ...
    protected void runIndexedArrayBF16(
        ...
        for (int outIndex = start; outIndex < end; outIndex++) {
            out[cursor.offset(0)] = TensorDTypeOps.toBFloat16Bits(
                    loadBF16(continuation, in, cursor.offset(1)) * parameter
            );
            if (outIndex + 1 < end) {
                cursor.step();
            }
    ...
    protected void runIndexedArrayBF16ToFloat(
        ...
        ElementwiseOffsetCursor cursor = layout.cursor(start);
        for (int outIndex = start; outIndex < end; outIndex++) {
            out[outIndex] = loadBF16(continuation, in, cursor.offset(1)) * parameter;
            if (outIndex + 1 < end) {
                cursor.step();
            }
        }
    ...
    protected void runIndexedSegmentF64(
        ...
        for (int outIndex = start; outIndex < end; outIndex++) {
            out.set(
                    ...
                    in.get(JAVA_DOUBLE, (long) cursor.offset(1) * Double.BYTES) * parameter
            );
            if (outIndex + 1 < end) {
                cursor.step();
            }
    ...
    protected void runIndexedSegmentF32(
        ...
        for (int outIndex = start; outIndex < end; outIndex++) {
            out.set(
                    ...
                    in.get(JAVA_FLOAT, (long) cursor.offset(1) * Float.BYTES) * parameter
            );
            if (outIndex + 1 < end) {
                cursor.step();
            }
    ...
    protected void runIndexedSegmentBF16(
        ...
        for (int outIndex = start; outIndex < end; outIndex++) {
            ...
            out.set(
                    ...
                    TensorDTypeOps.toBFloat16Bits(value * parameter)
            );
            if (outIndex + 1 < end) {
                cursor.step();
            }
    ...
    protected void runIndexedMixedF64(
        ...
        ElementwiseOffsetCursor cursor = layout.cursor(start);
        for (int outIndex = start; outIndex < end; outIndex++) {
            writeF64(out, cursor.offset(0), readF64(in, cursor.offset(1)) * parameter);
            if (outIndex + 1 < end) {
                cursor.step();
            }
        }
    ...
    protected void runIndexedMixedF32(
        ...
        ElementwiseOffsetCursor cursor = layout.cursor(start);
        for (int outIndex = start; outIndex < end; outIndex++) {
            writeF32(out, cursor.offset(0), readF32(in, cursor.offset(1)) * parameter);
            if (outIndex + 1 < end) {
                cursor.step();
            }
        }
    ...
    protected void runIndexedMixedBF16(
        ...
        ElementwiseOffsetCursor cursor = layout.cursor(start);
        for (int outIndex = start; outIndex < end; outIndex++) {
            writeBF16(out, cursor.offset(0), readBF16(in, cursor.offset(1)) * parameter);
            if (outIndex + 1 < end) {
                cursor.step();
            }
        }
