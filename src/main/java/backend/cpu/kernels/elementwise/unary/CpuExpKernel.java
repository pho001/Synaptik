...
public final class CpuExpKernel extends StorageAwareUnaryElementwiseKernel {
    ...
    protected StorageAwareUnaryElementwiseKernel selectExecutionKernel(CpuKernelContext context) {
        return context != null && context.useFastExpApprox() ? FAST : this;
    }

    @Override
    protected Operation.OpType opType() {
        return Operation.OpType.EXP;
    }
    ...
    private static void runArrayF64(double[] in, double[] out, int start, int end) {
        for (int i = start; i < end; i++) out[i] = Math.exp(in[i]);
    }
    ...
    private static void runArrayVectorF64(double[] in, double[] out, int start, int end) {
        ...
        int i = start;
        for (; i < upper; i += width) {
            DoubleVector.fromArray(F64, in, i).lanewise(VectorOperators.EXP).intoArray(out, i);
        }
        runArrayF64(in, out, i, end);
    }

    private static void runArrayF32(float[] in, float[] out, int start, int end) {
        for (int i = start; i < end; i++) out[i] = (float) Math.exp(in[i]);
    }
    ...
    private static void runArrayVectorF32(float[] in, float[] out, int start, int end) {
        ...
        int i = start;
        for (; i < upper; i += width) {
            FloatVector.fromArray(F32, in, i).lanewise(VectorOperators.EXP).intoArray(out, i);
        }
        runArrayF32(in, out, i, end);
    }

    private static void runArrayBF16(float[] in, short[] out, int start, int end) {
        for (int i = start; i < end; i++) out[i] = TensorDTypeOps.toBFloat16Bits((float) Math.exp(in[i]));
    }

    private static void runArrayBF16(short[] in, short[] out, int start, int end) {
        for (int i = start; i < end; i++) {
            out[i] = TensorDTypeOps.toBFloat16Bits((float) Math.exp(TensorDTypeOps.fromBFloat16Bits(in[i])));
        }
    }

    private static void runArrayBF16ToFloat(short[] in, float[] out, int start, int end) {
        for (int i = start; i < end; i++) out[i] = (float) Math.exp(TensorDTypeOps.fromBFloat16Bits(in[i]));
    }
    ...
    protected void runSegmentBF16(MemorySegment in, MemorySegment out, int start, int end) {
        for (int i = start; i < end; i++) {
            ...
            out.set(JAVA_SHORT, offset, TensorDTypeOps.toBFloat16Bits((float) Math.exp(value)));
        }
    }
    ...
    protected void runIndexedArrayF64(double[] in, double[] out, UnaryStorageLayout layout, int start, int end) {
        ...
        for (int outIndex = start; outIndex < end; outIndex++) {
            out[cursor.offset(0)] = Math.exp(in[cursor.offset(1)]);
            if (outIndex + 1 < end) {
                cursor.step();
            }
        }
    ...
    protected void runIndexedArrayF32(float[] in, float[] out, UnaryStorageLayout layout, int start, int end) {
        ElementwiseOffsetCursor cursor = layout.cursor(start);
        for (int outIndex = start; outIndex < end; outIndex++) {
            out[cursor.offset(0)] = (float) Math.exp(in[cursor.offset(1)]);
            if (outIndex + 1 < end) {
                cursor.step();
            }
        }
    ...
    protected void runIndexedArrayBF16(
        ...
        for (int outIndex = start; outIndex < end; outIndex++) {
            out[cursor.offset(0)] = TensorDTypeOps.toBFloat16Bits(
                    (float) Math.exp(loadBF16(continuation, in, cursor.offset(1)))
            );
            if (outIndex + 1 < end) {
                cursor.step();
            }
    ...
    protected void runIndexedArrayBF16ToFloat(
        ...
        ElementwiseOffsetCursor cursor = layout.cursor(start);
        for (int outIndex = start; outIndex < end; outIndex++) {
            out[outIndex] = (float) Math.exp(loadBF16(continuation, in, cursor.offset(1)));
            if (outIndex + 1 < end) {
                cursor.step();
            }
        }
    ...
    protected void runIndexedSegmentF64(
        ...
        for (int outIndex = start; outIndex < end; outIndex++) {
            out.set(
                    ...
                    Math.exp(in.get(JAVA_DOUBLE, (long) cursor.offset(1) * Double.BYTES))
            );
            if (outIndex + 1 < end) {
                cursor.step();
            }
    ...
    protected void runIndexedSegmentF32(
        ...
        for (int outIndex = start; outIndex < end; outIndex++) {
            out.set(
                    ...
                    (float) Math.exp(in.get(JAVA_FLOAT, (long) cursor.offset(1) * Float.BYTES))
            );
            if (outIndex + 1 < end) {
                cursor.step();
            }
    ...
    protected void runIndexedSegmentBF16(
        ...
        for (int outIndex = start; outIndex < end; outIndex++) {
            ...
            out.set(
                    ...
                    TensorDTypeOps.toBFloat16Bits((float) Math.exp(value))
            );
            if (outIndex + 1 < end) {
                cursor.step();
            }
    ...
    protected void runIndexedMixedF64(
        ...
        ElementwiseOffsetCursor cursor = layout.cursor(start);
        for (int outIndex = start; outIndex < end; outIndex++) {
            writeF64(out, cursor.offset(0), Math.exp(readF64(in, cursor.offset(1))));
            if (outIndex + 1 < end) {
                cursor.step();
            }
        }
    ...
    protected void runIndexedMixedF32(
        ...
        ElementwiseOffsetCursor cursor = layout.cursor(start);
        for (int outIndex = start; outIndex < end; outIndex++) {
            writeF32(out, cursor.offset(0), (float) Math.exp(readF32(in, cursor.offset(1))));
            if (outIndex + 1 < end) {
                cursor.step();
            }
        }
    ...
    protected void runIndexedMixedBF16(
        ...
        ElementwiseOffsetCursor cursor = layout.cursor(start);
        for (int outIndex = start; outIndex < end; outIndex++) {
            writeBF16(out, cursor.offset(0), (float) Math.exp(readBF16(in, cursor.offset(1))));
            if (outIndex + 1 < end) {
                cursor.step();
            }
        }
