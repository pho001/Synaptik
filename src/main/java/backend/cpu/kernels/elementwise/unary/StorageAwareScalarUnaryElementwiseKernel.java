...
abstract class StorageAwareScalarUnaryElementwiseKernel implements CpuStorageAwareKernel {
    @Override
    public final CpuKernelResult execute(CpuKernelCall call) {
        requireUnaryCall(call);
        ScalarUnaryParameters parameters = extractParameters(call.operation());
        if (ElementwiseNativeSupport.nativeRequested(call.context())) {
            return executeNative(call, parameters);
        }
        executeStorage(call.inputs().getFirst(), call.output(), call.context(), parameters);
    ...
    protected abstract ScalarUnaryParameters extractParameters(Operation operation);
    ...
    protected abstract void runDirectF64(
            double[] in,
            double parameter,
            double[] out,
    ...
    protected abstract void runDirectF32(
            float[] in,
            float parameter,
            float[] out,
            ResolvedDispatchHints hints
    ...
    protected abstract void runSegmentF64(
            MemorySegment in,
            double parameter,
            MemorySegment out,
            int start,
    ...
    protected abstract void runSegmentF32(
            MemorySegment in,
            float parameter,
            MemorySegment out,
            int start,
    ...
    protected abstract void runSegmentBF16(
            MemorySegment in,
            float parameter,
            MemorySegment out,
            int start,
    ...
    private CpuKernelResult executeNative(CpuKernelCall call, ScalarUnaryParameters parameters) {
        String ineligibleReason = nativeIneligibleReason(call);
        if (!ineligibleReason.isBlank()) {
            return fallbackToArray(call, ineligibleReason, parameters);
        }
        ...
        try {
            ...
            executeStorage(input, out, context, parameters);
    ...
    private CpuKernelResult fallbackToArray(
            CpuKernelCall call,
            String reason,
            ScalarUnaryParameters parameters
    ) {
        CpuKernelContext context = call.context();
        ...
        executeStorage(storage.input(0), storage.output(), context, parameters);
    ...
    private void executeStorage(
        ...
        switch (out.dtype()) {
            case FLOAT64 -> executeF64(input, out, context.dispatchHints(), parameters.f64());
            case FLOAT32 -> executeF32(input, out, context.dispatchHints(), parameters.f32());
            case BFLOAT16 -> executeBF16(input, out, context, parameters.f32());
    ...
    private void executeF64(
        ...
        if (canUseDenseDirectArray(input, out)) {
            runDirectF64(input.requireF64Array(), parameter, out.requireF64Array(), hints);
            return;
        }
        if (canUseDenseDirectSegment(input, out)) {
            ...
            ElementwiseRangeLoop.runScalar(length, hints,
                    (start, end) -> runSegmentF64(inputSegment, parameter, outSegment, start, end));
            return;
        }

        ScalarUnaryStorageLayout layout = ScalarUnaryStorageLayout.from(input, out);
        if (allArrays(input, out)) {
            double[] inputArray = input.requireF64Array();
            double[] outArray = out.requireF64Array();
            ElementwiseRangeLoop.runScalar(length, hints,
                    (start, end) -> runIndexedArrayF64(inputArray, parameter, outArray, layout, start, end));
            return;
        }
        if (allSegments(input, out)) {
            ...
            MemorySegment outSegment = out.requireSegment();
            ElementwiseRangeLoop.runScalar(length, hints,
                    (start, end) -> runIndexedSegmentF64(inputSegment, parameter, outSegment, layout, start, end));
            return;
        }
        ElementwiseRangeLoop.runScalar(length, hints,
                (start, end) -> runIndexedMixedF64(input, parameter, out, layout, start, end));
    }
    ...
    private void executeF32(
        ...
        int length = out.logicalSize();
        if (canUseDenseDirectArray(input, out)) {
            runDirectF32(input.requireF32Array(), parameter, out.requireF32Array(), hints);
            return;
        }
        if (canUseDenseDirectSegment(input, out)) {
            MemorySegment inputSegment = input.requireSegment();
            MemorySegment outSegment = out.requireSegment();
            ElementwiseRangeLoop.runScalar(length, hints,
                    (start, end) -> runSegmentF32(inputSegment, parameter, outSegment, start, end));
            return;
        }
        ...
        if (allArrays(input, out)) {
            ...
            ElementwiseRangeLoop.runScalar(length, hints,
                    (start, end) -> runIndexedArrayF32(inputArray, parameter, outArray, layout, start, end));
            return;
        }
        if (allSegments(input, out)) {
            MemorySegment inputSegment = input.requireSegment();
            MemorySegment outSegment = out.requireSegment();
            ElementwiseRangeLoop.runScalar(length, hints,
                    (start, end) -> runIndexedSegmentF32(inputSegment, parameter, outSegment, layout, start, end));
            return;
        }
        ElementwiseRangeLoop.runScalar(length, hints,
                (start, end) -> runIndexedMixedF32(input, parameter, out, layout, start, end));
    }
    ...
    private void executeBF16(
        ...
        if (canUseDenseDirectArray(input, out)) {
            ...
            if (canPublishFloatContinuation(context)) {
                ...
                runDirectBF16ToFloat(inputArray, continuation, parameter, outFloat, hints);
                context.cpuWorkspace().publishFloatContinuation(length);
                return;
            }
            runDirectBF16(inputArray, continuation, parameter, out.requireBF16Array(), hints);
            return;
        }
        if (canUseDenseDirectSegment(input, out)) {
            MemorySegment inputSegment = input.requireSegment();
            MemorySegment outSegment = out.requireSegment();
            ElementwiseRangeLoop.runScalar(length, hints,
                    (start, end) -> runSegmentBF16(inputSegment, parameter, outSegment, start, end));
            return;
        }
        ...
        if (allSegments(input, out)) {
            ...
            MemorySegment outSegment = out.requireSegment();
            ElementwiseRangeLoop.runScalar(length, hints,
                    (start, end) -> runIndexedSegmentBF16(inputSegment, parameter, outSegment, layout, start, end));
            return;
        }
        ElementwiseRangeLoop.runScalar(length, hints,
                (start, end) -> runIndexedMixedBF16(input, parameter, out, layout, start, end));
    }
    ...
    protected record ScalarUnaryParameters(double f64, float f32) {
    }

    protected record ScalarUnaryStorageLayout(int[] shape, int[][] cursorStrides, int[] cursorBaseOffsets) {
        static ScalarUnaryStorageLayout from(CpuStorageView input, CpuStorageView out) {
            int[] outputShape = out.shape();
            return new ScalarUnaryStorageLayout(
                    outputShape,
                    new int[][]{
                            out.strides(),
        ...
        protected ScalarUnaryStorageLayout {
            shape = shape.clone();
