...
public final class CpuInvKernel extends StorageAwareUnaryElementwiseKernel {
    ...
    private static void runArrayF64(double[] in, double[] out, int start, int end) {
        for (int i = start; i < end; i++) out[i] = 1.0d / in[i];
    }
    ...
    private static void runArrayVectorF64(double[] in, double[] out, int start, int end) {
        ...
        for (; i < upper; i += width) {
            one.div(DoubleVector.fromArray(F64, in, i)).intoArray(out, i);
        }
        runArrayF64(in, out, i, end);
    }

    private static void runArrayF32(float[] in, float[] out, int start, int end) {
        for (int i = start; i < end; i++) out[i] = 1.0f / in[i];
    }
    ...
    private static void runArrayVectorF32(float[] in, float[] out, int start, int end) {
        ...
        for (; i < upper; i += width) {
            one.div(FloatVector.fromArray(F32, in, i)).intoArray(out, i);
        }
        runArrayF32(in, out, i, end);
    }

    private static void runArrayBF16(float[] in, short[] out, int start, int end) {
        for (int i = start; i < end; i++) out[i] = TensorDTypeOps.toBFloat16Bits(1.0f / in[i]);
    }

    private static void runArrayBF16(short[] in, short[] out, int start, int end) {
        for (int i = start; i < end; i++) {
            out[i] = TensorDTypeOps.toBFloat16Bits(1.0f / TensorDTypeOps.fromBFloat16Bits(in[i]));
        }
    }

    private static void runArrayBF16ToFloat(short[] in, float[] out, int start, int end) {
        for (int i = start; i < end; i++) out[i] = 1.0f / TensorDTypeOps.fromBFloat16Bits(in[i]);
    }
    ...
    protected void runSegmentF64(MemorySegment in, MemorySegment out, int start, int end) {
        for (int i = start; i < end; i++) {
            ...
            out.set(JAVA_DOUBLE, offset, 1.0d / in.get(JAVA_DOUBLE, offset));
        }
    }
    ...
    protected void runSegmentF32(MemorySegment in, MemorySegment out, int start, int end) {
        for (int i = start; i < end; i++) {
            ...
            out.set(JAVA_FLOAT, offset, 1.0f / in.get(JAVA_FLOAT, offset));
        }
    }
    ...
    protected void runSegmentBF16(MemorySegment in, MemorySegment out, int start, int end) {
        for (int i = start; i < end; i++) {
            ...
            out.set(JAVA_SHORT, offset, TensorDTypeOps.toBFloat16Bits(1.0f / value));
        }
    }
    ...
    protected void runIndexedArrayF64(double[] in, double[] out, UnaryStorageLayout layout, int start, int end) {
        ...
        for (int outIndex = start; outIndex < end; outIndex++) {
            out[cursor.offset(0)] = 1.0d / in[cursor.offset(1)];
            if (outIndex + 1 < end) {
                cursor.step();
            }
        }
    ...
    protected void runIndexedArrayF32(float[] in, float[] out, UnaryStorageLayout layout, int start, int end) {
        ElementwiseOffsetCursor cursor = layout.cursor(start);
        for (int outIndex = start; outIndex < end; outIndex++) {
            out[cursor.offset(0)] = 1.0f / in[cursor.offset(1)];
            if (outIndex + 1 < end) {
                cursor.step();
            }
        }
    ...
    protected void runIndexedArrayBF16(
        ...
        for (int outIndex = start; outIndex < end; outIndex++) {
            out[cursor.offset(0)] = TensorDTypeOps.toBFloat16Bits(
                    1.0f / loadBF16(continuation, in, cursor.offset(1))
            );
            if (outIndex + 1 < end) {
                cursor.step();
            }
    ...
    protected void runIndexedArrayBF16ToFloat(
        ...
        ElementwiseOffsetCursor cursor = layout.cursor(start);
        for (int outIndex = start; outIndex < end; outIndex++) {
            out[outIndex] = 1.0f / loadBF16(continuation, in, cursor.offset(1));
            if (outIndex + 1 < end) {
                cursor.step();
            }
        }
    ...
    protected void runIndexedSegmentF64(
        ...
        for (int outIndex = start; outIndex < end; outIndex++) {
            out.set(
                    ...
                    1.0d / in.get(JAVA_DOUBLE, (long) cursor.offset(1) * Double.BYTES)
            );
            if (outIndex + 1 < end) {
                cursor.step();
            }
    ...
    protected void runIndexedSegmentF32(
        ...
        for (int outIndex = start; outIndex < end; outIndex++) {
            out.set(
                    ...
                    1.0f / in.get(JAVA_FLOAT, (long) cursor.offset(1) * Float.BYTES)
            );
            if (outIndex + 1 < end) {
                cursor.step();
            }
    ...
    protected void runIndexedSegmentBF16(
        ...
        for (int outIndex = start; outIndex < end; outIndex++) {
            ...
            out.set(
                    ...
                    TensorDTypeOps.toBFloat16Bits(1.0f / value)
            );
            if (outIndex + 1 < end) {
                cursor.step();
            }
    ...
    protected void runIndexedMixedF64(
        ...
        ElementwiseOffsetCursor cursor = layout.cursor(start);
        for (int outIndex = start; outIndex < end; outIndex++) {
            writeF64(out, cursor.offset(0), 1.0d / readF64(in, cursor.offset(1)));
            if (outIndex + 1 < end) {
                cursor.step();
            }
        }
    ...
    protected void runIndexedMixedF32(
        ...
        ElementwiseOffsetCursor cursor = layout.cursor(start);
        for (int outIndex = start; outIndex < end; outIndex++) {
            writeF32(out, cursor.offset(0), 1.0f / readF32(in, cursor.offset(1)));
            if (outIndex + 1 < end) {
                cursor.step();
            }
        }
    ...
    protected void runIndexedMixedBF16(
        ...
        ElementwiseOffsetCursor cursor = layout.cursor(start);
        for (int outIndex = start; outIndex < end; outIndex++) {
            writeBF16(out, cursor.offset(0), 1.0f / readBF16(in, cursor.offset(1)));
            if (outIndex + 1 < end) {
                cursor.step();
            }
        }
