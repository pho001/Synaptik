...
public final class CpuFastTanhKernel extends StorageAwareUnaryElementwiseKernel {
    @Override
    protected Operation.OpType opType() {
        return Operation.OpType.FAST_TANH;
    }
    ...
    private static void runArrayF64(double[] in, double[] out, int start, int end) {
        for (int i = start; i < end; i++) out[i] = FastTranscendentals.fastTanhF64(in[i]);
    }

    private static void runArrayF32(float[] in, float[] out, int start, int end) {
        for (int i = start; i < end; i++) out[i] = FastTranscendentals.fastTanhF32(in[i]);
    }

    private static void runArrayBF16(float[] in, short[] out, int start, int end) {
        for (int i = start; i < end; i++) {
            out[i] = TensorDTypeOps.toBFloat16Bits(FastTranscendentals.fastTanhF32(in[i]));
        }
    }
    ...
    private static void runArrayBF16(short[] in, short[] out, int start, int end) {
        for (int i = start; i < end; i++) {
            out[i] = TensorDTypeOps.toBFloat16Bits(
                    FastTranscendentals.fastTanhF32(TensorDTypeOps.fromBFloat16Bits(in[i]))
            );
        }
    }
    ...
    private static void runArrayBF16ToFloat(short[] in, float[] out, int start, int end) {
        for (int i = start; i < end; i++) {
            out[i] = FastTranscendentals.fastTanhF32(TensorDTypeOps.fromBFloat16Bits(in[i]));
        }
    }
    ...
    protected void runSegmentF64(MemorySegment in, MemorySegment out, int start, int end) {
        for (int i = start; i < end; i++) {
            ...
            out.set(JAVA_DOUBLE, offset, FastTranscendentals.fastTanhF64(in.get(JAVA_DOUBLE, offset)));
        }
    }
    ...
    protected void runSegmentF32(MemorySegment in, MemorySegment out, int start, int end) {
        for (int i = start; i < end; i++) {
            ...
            out.set(JAVA_FLOAT, offset, FastTranscendentals.fastTanhF32(in.get(JAVA_FLOAT, offset)));
        }
    }
    ...
    protected void runSegmentBF16(MemorySegment in, MemorySegment out, int start, int end) {
        for (int i = start; i < end; i++) {
            ...
            out.set(JAVA_SHORT, offset, TensorDTypeOps.toBFloat16Bits(FastTranscendentals.fastTanhF32(value)));
        }
    }
    ...
    protected void runIndexedArrayF64(double[] in, double[] out, UnaryStorageLayout layout, int start, int end) {
        ...
        for (int outIndex = start; outIndex < end; outIndex++) {
            out[cursor.offset(0)] = FastTranscendentals.fastTanhF64(in[cursor.offset(1)]);
            if (outIndex + 1 < end) {
                cursor.step();
            }
        }
    ...
    protected void runIndexedArrayF32(float[] in, float[] out, UnaryStorageLayout layout, int start, int end) {
        ElementwiseOffsetCursor cursor = layout.cursor(start);
        for (int outIndex = start; outIndex < end; outIndex++) {
            out[cursor.offset(0)] = FastTranscendentals.fastTanhF32(in[cursor.offset(1)]);
            if (outIndex + 1 < end) {
                cursor.step();
            }
        }
    ...
    protected void runIndexedArrayBF16(
        ...
        for (int outIndex = start; outIndex < end; outIndex++) {
            out[cursor.offset(0)] = TensorDTypeOps.toBFloat16Bits(
                    FastTranscendentals.fastTanhF32(loadBF16(continuation, in, cursor.offset(1)))
            );
            if (outIndex + 1 < end) {
                cursor.step();
            }
    ...
    protected void runIndexedArrayBF16ToFloat(
        ...
        ElementwiseOffsetCursor cursor = layout.cursor(start);
        for (int outIndex = start; outIndex < end; outIndex++) {
            out[outIndex] = FastTranscendentals.fastTanhF32(loadBF16(continuation, in, cursor.offset(1)));
            if (outIndex + 1 < end) {
                cursor.step();
            }
        }
    ...
    protected void runIndexedSegmentF64(
        ...
        for (int outIndex = start; outIndex < end; outIndex++) {
            out.set(
                    ...
                    FastTranscendentals.fastTanhF64(in.get(JAVA_DOUBLE, (long) cursor.offset(1) * Double.BYTES))
            );
            if (outIndex + 1 < end) {
                cursor.step();
            }
    ...
    protected void runIndexedSegmentF32(
        ...
        for (int outIndex = start; outIndex < end; outIndex++) {
            out.set(
                    ...
                    FastTranscendentals.fastTanhF32(in.get(JAVA_FLOAT, (long) cursor.offset(1) * Float.BYTES))
            );
            if (outIndex + 1 < end) {
                cursor.step();
            }
    ...
    protected void runIndexedSegmentBF16(
        ...
        for (int outIndex = start; outIndex < end; outIndex++) {
            ...
            out.set(
                    ...
                    TensorDTypeOps.toBFloat16Bits(FastTranscendentals.fastTanhF32(value))
            );
            if (outIndex + 1 < end) {
                cursor.step();
            }
    ...
    protected void runIndexedMixedF64(
        ...
        ElementwiseOffsetCursor cursor = layout.cursor(start);
        for (int outIndex = start; outIndex < end; outIndex++) {
            writeF64(out, cursor.offset(0), FastTranscendentals.fastTanhF64(readF64(in, cursor.offset(1))));
            if (outIndex + 1 < end) {
                cursor.step();
            }
        }
    ...
    protected void runIndexedMixedF32(
        ...
        ElementwiseOffsetCursor cursor = layout.cursor(start);
        for (int outIndex = start; outIndex < end; outIndex++) {
            writeF32(out, cursor.offset(0), FastTranscendentals.fastTanhF32(readF32(in, cursor.offset(1))));
            if (outIndex + 1 < end) {
                cursor.step();
            }
        }
    ...
    protected void runIndexedMixedBF16(
        ...
        ElementwiseOffsetCursor cursor = layout.cursor(start);
        for (int outIndex = start; outIndex < end; outIndex++) {
            writeBF16(out, cursor.offset(0), FastTranscendentals.fastTanhF32(readBF16(in, cursor.offset(1))));
            if (outIndex + 1 < end) {
                cursor.step();
            }
        }
