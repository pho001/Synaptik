...
public final class CpuDivKernel extends StorageAwareBinaryElementwiseKernel {
    ...
    private static void runArrayVectorF64(double[] left, double[] right, double[] out, int start, int end) {
        ...
        int i = start;
        for (; i < upper; i += width) {
            DoubleVector.fromArray(F64, left, i).div(DoubleVector.fromArray(F64, right, i)).intoArray(out, i);
        }
    ...
    private static void runArrayVectorF32(float[] left, float[] right, float[] out, int start, int end) {
        ...
        int i = start;
        for (; i < upper; i += width) {
            FloatVector.fromArray(F32, left, i).div(FloatVector.fromArray(F32, right, i)).intoArray(out, i);
        }
