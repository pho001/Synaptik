...
public final class CpuSubKernel extends StorageAwareBinaryElementwiseKernel {
    ...
    private static void runArrayF64(double[] left, double[] right, double[] out, int start, int end) {
        for (int i = start; i < end; i++) out[i] = left[i] - right[i];
    }
    ...
    private static void runArrayVectorF64(double[] left, double[] right, double[] out, int start, int end) {
        ...
        int i = start;
        for (; i < upper; i += width) {
            DoubleVector.fromArray(F64, left, i).sub(DoubleVector.fromArray(F64, right, i)).intoArray(out, i);
        }
    ...
    private static void runArrayVectorF32(float[] left, float[] right, float[] out, int start, int end) {
        ...
        int i = start;
        for (; i < upper; i += width) {
            FloatVector.fromArray(F32, left, i).sub(FloatVector.fromArray(F32, right, i)).intoArray(out, i);
        }
        runArrayF32(left, right, out, i, end);
    }

    private static void runArrayBF16(float[] left, float[] right, short[] out, int start, int end) {
        for (int i = start; i < end; i++) out[i] = TensorDTypeOps.toBFloat16Bits(left[i] - right[i]);
    }
    ...
    private static void runArrayBF16(short[] left, short[] right, short[] out, int start, int end) {
        for (int i = start; i < end; i++) {
            ...
            out[i] = TensorDTypeOps.toBFloat16Bits(leftValue - rightValue);
        }
    }
