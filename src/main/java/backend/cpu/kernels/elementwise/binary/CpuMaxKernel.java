...
public final class CpuMaxKernel extends StorageAwareBinaryElementwiseKernel {
    ...
    private static void runArrayVectorF64(double[] left, double[] right, double[] out, int start, int end) {
        ...
        for (; i < upper; i += width) {
            DoubleVector.fromArray(F64, left, i)
                    .lanewise(VectorOperators.MAX, DoubleVector.fromArray(F64, right, i))
                    .intoArray(out, i);
        }
    ...
    private static void runArrayVectorF32(float[] left, float[] right, float[] out, int start, int end) {
        ...
        for (; i < upper; i += width) {
            FloatVector.fromArray(F32, left, i)
                    .lanewise(VectorOperators.MAX, FloatVector.fromArray(F32, right, i))
                    .intoArray(out, i);
        }
    ...
    private static void runArrayBF16(float[] left, short[] right, short[] out, int start, int end) {
        for (int i = start; i < end; i++) {
            out[i] = TensorDTypeOps.toBFloat16Bits(Math.max(left[i], TensorDTypeOps.fromBFloat16Bits(right[i])));
        }
    }

    private static void runArrayBF16(short[] left, float[] right, short[] out, int start, int end) {
        for (int i = start; i < end; i++) {
            out[i] = TensorDTypeOps.toBFloat16Bits(Math.max(TensorDTypeOps.fromBFloat16Bits(left[i]), right[i]));
        }
    }
    ...
    private static void runArrayBF16ToFloat(
        ...
        for (int i = start; i < end; i++) {
            out[i] = Math.max(loadBF16(leftContinuation, leftStorage, i), loadBF16(rightContinuation, rightStorage, i));
        }
    }
    ...
    protected void runIndexedArrayF64(
        ...
        for (int outIndex = start; outIndex < end; outIndex++) {
            out[cursor.offset(0)] = Math.max(left[cursor.offset(1)], right[cursor.offset(2)]);
            if (outIndex + 1 < end) {
                cursor.step();
            }
        }
    ...
    protected void runIndexedArrayBF16(
        ...
        for (int outIndex = start; outIndex < end; outIndex++) {
            ...
            out[cursor.offset(0)] = TensorDTypeOps.toBFloat16Bits(Math.max(leftValue, rightValue));
            if (outIndex + 1 < end) {
                cursor.step();
            }
        }
    ...
    protected void runIndexedSegmentF64(
        ...
        for (int outIndex = start; outIndex < end; outIndex++) {
            ...
            out.set(JAVA_DOUBLE, outOffset, Math.max(left.get(JAVA_DOUBLE, leftOffset), right.get(JAVA_DOUBLE, rightOffset)));
            if (outIndex + 1 < end) {
                cursor.step();
            }
        }
    ...
    protected void runIndexedSegmentF32(
        ...
        for (int outIndex = start; outIndex < end; outIndex++) {
            ...
            out.set(JAVA_FLOAT, outOffset, Math.max(left.get(JAVA_FLOAT, leftOffset), right.get(JAVA_FLOAT, rightOffset)));
            if (outIndex + 1 < end) {
                cursor.step();
            }
        }
    ...
    protected void runIndexedSegmentBF16(
        ...
        for (int outIndex = start; outIndex < end; outIndex++) {
            ...
            out.set(
                    ...
                    TensorDTypeOps.toBFloat16Bits(Math.max(leftValue, rightValue))
            );
            if (outIndex + 1 < end) {
                cursor.step();
            }
    ...
    protected void runIndexedMixedF64(
        ...
        ElementwiseOffsetCursor cursor = layout.cursor(start);
        for (int outIndex = start; outIndex < end; outIndex++) {
            writeF64(out, cursor.offset(0), Math.max(readF64(left, cursor.offset(1)), readF64(right, cursor.offset(2))));
            if (outIndex + 1 < end) {
                cursor.step();
            }
        }
    ...
    protected void runIndexedMixedF32(
        ...
        ElementwiseOffsetCursor cursor = layout.cursor(start);
        for (int outIndex = start; outIndex < end; outIndex++) {
            writeF32(out, cursor.offset(0), Math.max(readF32(left, cursor.offset(1)), readF32(right, cursor.offset(2))));
            if (outIndex + 1 < end) {
                cursor.step();
            }
        }
    ...
    protected void runIndexedMixedBF16(
        ...
        ElementwiseOffsetCursor cursor = layout.cursor(start);
        for (int outIndex = start; outIndex < end; outIndex++) {
            writeBF16(out, cursor.offset(0), Math.max(readBF16(left, cursor.offset(1)), readBF16(right, cursor.offset(2))));
            if (outIndex + 1 < end) {
                cursor.step();
            }
        }
