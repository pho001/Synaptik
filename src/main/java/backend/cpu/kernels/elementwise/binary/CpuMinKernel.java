...
public final class CpuMinKernel extends TypedCpuKernel implements BinaryElementwiseKernel {
    ...
    private static void runArrayVectorF64(double[] left, double[] right, double[] out, int start, int end) {
        ...
        int i = start;
        for (; i < upper; i += width) {
            DoubleVector.fromArray(F64, left, i).lanewise(VectorOperators.MIN, DoubleVector.fromArray(F64, right, i)).intoArray(out, i);
        }
    ...
    private static void runArrayVectorF32(float[] left, float[] right, float[] out, int start, int end) {
        ...
        int i = start;
        for (; i < upper; i += width) {
            FloatVector.fromArray(F32, left, i).lanewise(VectorOperators.MIN, FloatVector.fromArray(F32, right, i)).intoArray(out, i);
        }
    ...
    private static void runArrayBF16(float[] left, short[] right, short[] out, int start, int end) {
        for (int i = start; i < end; i++) out[i] = TensorDTypeOps.toBFloat16Bits(Math.min(left[i], TensorDTypeOps.fromBFloat16Bits(right[i])));
    }

    private static void runArrayBF16(short[] left, float[] right, short[] out, int start, int end) {
        for (int i = start; i < end; i++) out[i] = TensorDTypeOps.toBFloat16Bits(Math.min(TensorDTypeOps.fromBFloat16Bits(left[i]), right[i]));
    }
