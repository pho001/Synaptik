...
public final class Float32FusedExecutor {
    ...
    private static Object evalNumericVector(
        ...
        return switch (node.opType()) {
            ...
            case POW_TENSOR -> throw new UnsupportedOperationException("F32 fused vector tensor pow is intentionally disabled.");
            case NOOP -> a;
    ...
    private static float evalNumeric(FusedNodePlan node, float[] in, FusedExecutionOptions options) {
        ...
        return switch (node.opType()) {
            ...
            case EXP -> options.useFastExpApprox() ? backend.cpu.fused.codegen.FusedScalarOps.fastExpF32(in[0]) : backend.cpu.fused.codegen.FusedScalarOps.expF32(in[0], false);
            case FAST_EXP -> backend.cpu.fused.codegen.FusedScalarOps.fastExpF32(in[0]);
