...
final class PreparedInputPolicy {
    ...
    static boolean bypassPreparation(Operation op) {
        ...
        return switch (op.opType()) {
            case CONTIGUOUS, RESHAPE, EXPAND, SELECT, SLICE, SLICE_BACKWARD, CONCAT, PAD, TILE, UNFOLD_AXIS, PERMUTE, EXPAND_DIMS, SQUEEZE, CAST,
    ...
    static boolean requiresPreparedInput(
        ...
        if (input.hasStorageOffset()) {
            ...
            return switch (op.opType()) {
                case FUSED -> false;
                case RESHAPE, EXPAND, SELECT, SLICE, SLICE_BACKWARD, CONCAT, PAD, TILE, UNFOLD_AXIS, PERMUTE, EXPAND_DIMS, SQUEEZE, CAST,
                        GATHER, GATHER_AXIS,
