...
public final class StridedPathEligibility {
    ...
    public static StridedLayoutDecision resolve(
        ...
        for (int i = 0; i < inputs.size(); i++) {
            ...
            boolean sameShape = Arrays.equals(input.shape(), outShape);
            if (!sameShape && !canUseBroadcastShape(op, input.shape(), outShape, i)) {
                return StridedLayoutDecision.NONE;
            }
    ...
    private static boolean canUseBroadcastShape(Operation op, int[] inputShape, int[] outShape, int inputIndex) {
        if (op == null || op.opType() == null) {
            return false;
        }
        return switch (op.opType()) {
            case ADD, SUB, MUL, DIV, MIN, MAX, GT, GE, LT, LE, EQ, NE, LOGICAL_AND, LOGICAL_OR, WHERE ->
                    ElementwiseLayoutPlan.canBroadcastTo(inputShape, outShape);
            default -> inputIndex == 0 && Arrays.equals(inputShape, outShape);
        };
    }
}
