...
public final class MinF64 {
    private static final VectorSpecies<Double> SPECIES = DoubleVector.SPECIES_PREFERRED;

    private MinF64() {}

    public static void run(double[] a, double[] b, double[] out, CpuExecutionMode mode, CpuExecutionConfig config) {
        switch (mode) {
    ...
    private static void vector(double[] a, double[] b, double[] out) {
        ...
        for (; i < upper; i += SPECIES.length()) {
            DoubleVector.fromArray(SPECIES, a, i).min(DoubleVector.fromArray(SPECIES, b, i)).intoArray(out, i);
        }
        scalar(a, b, out, i, out.length);
    }

    private static void parallel(double[] a, double[] b, double[] out, CpuExecutionConfig config) {
    ...
    private static void parallelVector(double[] a, double[] b, double[] out, CpuExecutionConfig config) {
