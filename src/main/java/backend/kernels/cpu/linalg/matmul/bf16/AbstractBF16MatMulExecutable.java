...
abstract class AbstractBF16MatMulExecutable implements PreparedMatMulExecutable {
    ...
    private boolean tryPackedToFloat(
        ...
        if (leftContinuation != null) {
            backend.kernels.cpu.linalg.matmul.f32.F32MatMulJavaBackend.runPacked(leftContinuation, a.getShapeUnsafe(), packed, out, node.getShapeUnsafe(), hints);
        } else {
    ...
    private boolean tryContinuationToFloat(
        ...
        if (leftContinuation != null && rightContinuation != null) {
            backend.kernels.cpu.linalg.matmul.f32.F32MatMulJavaBackend.run(leftContinuation, as, rightContinuation, bs, out, node.getShapeUnsafe(), hints);
            workspace.publishFloatContinuation(node.getFlatDataSize());
            return true;
        }
        if (leftContinuation != null) {
