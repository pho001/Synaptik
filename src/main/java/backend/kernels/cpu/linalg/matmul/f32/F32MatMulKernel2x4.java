...
final class F32MatMulKernel2x4 {
    private F32MatMulKernel2x4() {
    }
    ...
    static void computeBlockF32_2x4(
        ...
        for (int kk = kStart; kk < kEnd; kk += tk) {
            ...
            for (int jj = jStart; jj < jEnd; jj += tn) {
                ...
                int i = iStart;
                for (; i + 1 < iEnd; i += 2) {
                    F32MatMulKernel4ColsSupport.computeTwoRowsFourColsF32(a, out, packedB, aOffset, outOffset, i, jj, kk, kkEnd, n, k, panelWidth, blockLimit, vectorLimit, width);
                }
                for (; i < iEnd; i++) {
        ...
        for (int kk = kStart; kk < kEnd; kk += tk) {
            ...
            for (; i + 1 < iEnd; i += 2) {
                ...
                for (int jj = jStart; jj < jEnd; jj += tn) {
                    ...
                    int blockLimit = panelWidth - (panelWidth % vectorBlockWidth);
                    float[] packedB = F32MatMulPacking.packedPanelF32(b, bOffset, kk, kkEnd, jj, jjEnd, n);
                    F32MatMulKernel4ColsSupport.computeTwoRowsFourColsF32(packedA, out, packedB, 0, tileOutOffset, 0, jj, 0, panelDepth, n, panelDepth, panelWidth, blockLimit, vectorLimit, width);
                }
            }
            for (; i < iEnd; i++) {
