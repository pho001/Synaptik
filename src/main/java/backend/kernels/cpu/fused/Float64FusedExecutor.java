...
import backend.cpu.fused.codegen.FusedScalarOps;
...
public final class Float64FusedExecutor {
    ...
    private static double evalNumeric(FusedNodePlan node, double[] in, FusedExecutionOptions options) {
        ...
        return switch (node.opType()) {
            ...
            case EXP -> options.useFastExpApprox() ? backend.cpu.fused.codegen.FusedScalarOps.fastExpF64(in[0]) : backend.cpu.fused.codegen.FusedScalarOps.expF64(in[0], false);
            case FAST_EXP -> backend.cpu.fused.codegen.FusedScalarOps.fastExpF64(in[0]);
