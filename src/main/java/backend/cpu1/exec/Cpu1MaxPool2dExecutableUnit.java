...
/**
 * Runtime wrapper for a prepared cpu1 MAX_POOL2D node.
 */
public final class Cpu1MaxPool2dExecutableUnit implements Cpu1ExecutableUnit {
    private final Cpu1PreparedMaxPool2dUnit preparedUnit;

    public Cpu1MaxPool2dExecutableUnit(Cpu1PreparedMaxPool2dUnit preparedUnit) {
    ...
    public void run(ExecutionContext context) {
        ...
        if (preparedUnit.storageKind() == Cpu1StorageKind.JAVA_ARRAY) {
            ...
            nativeOutput = context.requireNativeOutputStorage(
                    ...
                    "cpu1-maxpool2d-node-" + preparedUnit.nodeId()
            );
            ...
            throw new UnsupportedOperationException("cpu1 MAX_POOL2D executable does not support storage "
        ...
        if (nativeOutput == null) {
            ...
            context.markCpuCurrent(preparedUnit.nodeId(), "cpu1 MAX_POOL2D wrote CPU array");
        } else {
            nativeOutput.markModified();
            context.attachNativeStorage(
                    preparedUnit.nodeId(),
                    nativeOutput,
                    "cpu1 MAX_POOL2D wrote native CPU segment"
            );
        }
    }
