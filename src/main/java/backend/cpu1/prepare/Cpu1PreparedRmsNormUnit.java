...
import backend.cpu1.kernels.nn.normalization.rmsnorm.Cpu1RmsNormKernel;
import backend.cpu1.kernels.nn.normalization.rmsnorm.Cpu1RmsNormKernelDispatch;
...
/**
 * Immutable prepare-time contract for one dense cpu1 RMSNorm node.
 */
public final class Cpu1PreparedRmsNormUnit {
    ...
    private final Cpu1RmsNormKernelId kernelId;
    private final Cpu1RmsNormKernel kernel;
    ...
    public Cpu1PreparedRmsNormUnit(
            int nodeId,
            int inputNodeId,
            int gammaNodeId,
        ...
        if (nodeId < 0 || inputNodeId < 0 || gammaNodeId < 0) {
            throw new IllegalArgumentException("node ids cannot be negative");
        }
        if (opType != Operation.OpType.RMS_NORM) {
            throw new IllegalArgumentException("Cpu1PreparedRmsNormUnit requires RMS_NORM op, got " + opType);
        }
        ...
        this.kernel = Cpu1RmsNormKernelDispatch.kernelFor(kernelId);
    ...
    public Cpu1RmsNormKernelId kernelId() {
        return kernelId;
    }

    public Cpu1RmsNormKernel kernel() {
        return kernel;
    }
