...
import backend.cpu1.kernels.nn.pool.avgpool.Cpu1AvgPool2dKernel;
import backend.cpu1.kernels.nn.pool.avgpool.Cpu1AvgPool2dKernelDispatch;
...
/**
 * Immutable prepare-time contract for one dense cpu1 AVG_POOL2D node.
 */
public final class Cpu1PreparedAvgPool2dUnit {
    private final int nodeId;
    private final int inputNodeId;
    private final Operation.OpType opType;
    private final DataType dataType;
    private final Cpu1StorageKind storageKind;
    private final Cpu1AvgPool2dKernelId kernelId;
    private final Cpu1AvgPool2dKernel kernel;
    ...
    public Cpu1PreparedAvgPool2dUnit(
            int nodeId,
            int inputNodeId,
            Operation.OpType opType,
            DataType dataType,
            Cpu1StorageKind storageKind,
            Cpu1AvgPool2dKernelId kernelId,
            int[] inputShape,
            int[] outputShape,
        ...
        if (opType != Operation.OpType.AVG_POOL2D) {
            throw new IllegalArgumentException("Cpu1PreparedAvgPool2dUnit requires AVG_POOL2D op, got " + opType);
        }
        ...
        if (inputShape.length != 4 || outputShape.length != 4) {
            throw new IllegalArgumentException("AVG_POOL2D requires rank-4 NCHW shapes. input="
        ...
        if (inputShape[0] != outputShape[0] || inputShape[1] != outputShape[1]) {
            throw new IllegalArgumentException("AVG_POOL2D input/output N and C dimensions must match. input="
                    + Arrays.toString(inputShape) + ", output=" + Arrays.toString(outputShape));
        }
        ...
        this.kernel = Cpu1AvgPool2dKernelDispatch.kernelFor(kernelId);
    ...
    public Cpu1AvgPool2dKernelId kernelId() {
        return kernelId;
    }

    public Cpu1AvgPool2dKernel kernel() {
        return kernel;
    }
    ...
    public boolean countIncludePad() {
        return options.countIncludePad();
    }
