...
/**
 * Stable compact text renderer for lowered GPU region manifests.
 */
public final class GpuLoweredRegionManifestRenderer {
    private GpuLoweredRegionManifestRenderer() {
    }

    /**
     * Renders a deterministic multi-line manifest summary for human-readable traces and reports.
     *
     * @param manifest lowered GPU region manifest
     * @return compact manifest block, or an empty string when the manifest is absent
     */
    public static String renderCompact(GpuLoweredRegionManifest manifest) {
        if (manifest == null) {
            return "";
        }
        ...
        if (manifest.originalOps().isEmpty()) {
            ...
            for (GpuLoweredRegionOriginalOp op : manifest.originalOps()) {
    ...
    private static void appendAssumptions(
            StringBuilder sb,
            String prefix,
            java.util.List<GpuLoweredRegionValueAssumption> assumptions
    ) {
        if (assumptions == null || assumptions.isEmpty()) {
            sb.append("- ").append(prefix).append(": none\n");
            return;
        }
        for (GpuLoweredRegionValueAssumption assumption : assumptions) {
