...
public final class TensorReduceOps {
    ...
    /**
     * Sums values along one dimension while ignoring positions where {@code mask} is false.
     *
    ...
    public static Tensor sum(Tensor input, int dimension, Tensor mask) {
        ...
        Tensor alignedMask = alignMaskToShape(mask, input.getShapeUnsafe(), normalizedDimension, "masked sum");
    ...
    /**
     * Averages values along one dimension while ignoring positions where {@code mask} is false.
     *
    ...
    public static Tensor mean(Tensor input, int dimension, Tensor mask) {
        ...
        int normalizedDimension = TensorLayoutTransform.normalizeAxis(dimension, input.getShapeUnsafe().length);
        Tensor alignedMask = alignMaskToShape(mask, input.getShapeUnsafe(), normalizedDimension, "masked mean");
        Tensor maskedInput = Tensor.where(alignedMask, input, Tensor.zerosLike(input));
    ...
    private static Tensor alignMaskToShape(Tensor mask, int[] targetShape, int preferredAxis, String opName) {
        if (mask == null) {
