...
/**
 * Value tracked inside an planned partition.
 *
 * @param ref partition-scoped value reference
 ...
 * @param requiredMaterialized whether graph semantics require materialized storage at the partition boundary
 */
public record PartitionExecutionValue(
        GraphValueRef ref,
    ...
    public PartitionExecutionValue {
        if (ref == null || sourceValueRef == null || transportKind == null || typeContract == null) {
            throw new IllegalArgumentException("PartitionExecutionValue fields cannot be null");
        }
