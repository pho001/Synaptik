...
final class SdpaBackwardSpecializationDetector {
    ...
    static List<PartitionSpecializationCandidate> findCandidates(
        ...
        LinkedHashSet<PartitionSpecializationCandidate> out = new LinkedHashSet<>();
        for (int terminalNodeId : partition.orderedNodeIds()) {
            if (!partition.outputValueRefs().contains(GraphValueRef.node(terminalNodeId))) {
                continue;
            }
            PartitionSpecializationCandidate candidate = matchValueGradient(
                    terminalNodeId,
                    partition,
                    partitionNodeIds,
                    context
    ...
    private static PartitionSpecializationCandidate matchValueGradient(
            int terminalNodeId,
            Partition partition,
    ...
    private static PartitionSpecializationCandidate matchQueryOrKeyGradient(
            int terminalNodeId,
            Partition partition,
            Set<Integer> partitionNodeIds,
            PartitionExecutionPlanningContext context
    ...
    private static ScoreChain matchScoreChain(int dScoresNodeId, PartitionExecutionPlanningContext context) {
    ...
    private static PartitionSpecializationCandidate candidate(
            Partition partition,
    ...
    private static ForwardMetadata forwardMetadata(int weightsNodeId, PartitionExecutionPlanningContext context) {
    ...
    private static int transposedInputNodeId(int nodeId, PartitionExecutionPlanningContext context) {
