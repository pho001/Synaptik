...
public final class ExecutableProfileFingerprint {
    ...
    private static String canonicalSpec(ExecutionProfile profile) {
        ...
        sb.append("backend.cost.transferCostPreset=")
                ...
                .append('|');
        sb.append("cpuPartition.policy=").append(backend.cpuPartitions().policy().name()).append('|');
        sb.append("cpuPartition.maxPartitionNodes=").append(backend.cpuPartitions().maxPartitionNodes()).append('|');
        sb.append("cpuPartition.fanout=").append(backend.cpuPartitions().fanoutPolicy().name()).append('|');
        sb.append("cpuPartition.boundary=").append(backend.cpuPartitions().boundaryPolicy().name()).append('|');
        sb.append("partition.enabled=").append(partition.enabled()).append('|');
        sb.append("fuse.maxClusterNodes=").append(partition.fuse().maxClusterNodes()).append('|');
        sb.append("fuse.scoreThreshold=").append(fmt(partition.fuse().scoreThreshold())).append('|');
        sb.append("fuse.internalEdgeBonus=").append(fmt(partition.fuse().internalEdgeBonus())).append('|');
        sb.append("fuse.externalInputPenalty=").append(fmt(partition.fuse().externalInputPenalty())).append('|');
        sb.append("fuse.sharedExpensivePenalty=").append(fmt(partition.fuse().sharedExpensivePenalty())).append('|');
        sb.append("fuse.nonCheapBonus=").append(fmt(partition.fuse().nonCheapBonus())).append('|');
        sb.append("fuse.preserveSharedExpensiveNodes=").append(partition.fuse().preserveSharedExpensiveNodes()).append('|');
        sb.append("cpuFusion.mode=").append(partition.cpuFusion().mode().name()).append('|');
        sb.append("cpuFusion.maxChainNodes=").append(partition.cpuFusion().maxChainNodes()).append('|');
        sb.append("cpuFusion.fanout=").append(partition.cpuFusion().fanoutPolicy().name()).append('|');
        sb.append("cpuFusion.layout=").append(partition.cpuFusion().layoutPolicy().name()).append('|');
        sb.append("cpuFusion.cheapProducer=").append(partition.cpuFusion().cheapProducerPolicy().name()).append('|');
