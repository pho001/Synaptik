...
final class DefaultAutotuneSession implements AutotuneSession {
    ...
    public TuningResult run() {
        ...
        Candidate bestCandidate = finalists.isEmpty() ? null : finalists.getFirst().candidate();
        ExecutionProfile bestProfile = bestCandidate == null ? null : bestCandidate.profile();
        String summary = "evaluated=" + evaluated.size()
                + ", valid=" + finalists.size()
                + ", selected=" + seenFingerprints.size();
        boolean persisted = persist(bestCandidate, evaluated, finalists, summary);
    ...
    private boolean persist(
        ...
        String runtimeProfileId = request.runtimeProfile().metadata().platformProfileId();
        ...
        if (policy.persistHistory() && policy.historyPath() != null) {
            for (BenchmarkCandidateReport report : evaluated) {
                ...
                historyStore.append(policy.historyPath(), new TuningHistoryEntry(
                        ExecutableProfileFingerprint.of(report.candidate()),
                        ...
                        runtimeProfileId,
                        report.candidate().metadata().productionEligible()
                ));
            }
        }
        ...
        if (policy.persistBestProfile() && policy.bestProfilePath() != null && bestProfile != null) {
            ...
            bestProfileStore.save(policy.bestProfilePath(), new BestProfileRecord(
                    ...
                    java.time.OffsetDateTime.now(),
                    "autotune",
                    bestCandidate.metadata().graphAutotuneMode(),
                    bestCandidate.kind(),
                    bestCandidate.metadata(),
                    runtimeProfileId,
                    bestCandidate.metadata().productionEligible()
            ));
            return true;
        }
        return false;
