...
- [ ] **ABI-01**: Runtime device buffer bindings can represent backend id, native handle identity, dtype, shape, strides, storage offset, logical element count, byte length, and access mode.
- [ ] **ABI-02**: The shared accelerator buffer model is backend-neutral and reusable by Metal now and CUDA later.
- [ ] **ABI-03**: Buffer compatibility checks distinguish dense contiguous tensors, zero-offset views, non-zero-offset views, permuted/strided views, broadcast/zero-stride views, and unsupported layouts.
- [ ] **ABI-04**: Buffer binding decisions expose stable reason codes for success, fallback, unsupported dtype, unsupported layout, unavailable native ABI, and required-but-unavailable buffer execution.
