...
# Backend Planning And Partitions
...
This document explains compile-time backend ownership partitions, partition execution planning (configured through
`CompileConfig.partitionExecution()`), and memory planning. The current compile model is
`PlannedPartition` -> `ExecutablePartitionPlan(PlannedPartition, PartitionExecutionPlan)`; there is no separate
intermediate lifecycle identity between a partition and its execution plan.

## Term Map

| Term | Meaning |
|---|---|
| Backend | Runtime family that can execute a node or partition, such as CPU, Metal, or CUDA. |
| Backend intent | A compile-time hint on a node that says which backend should own it if legal. |
| Backend planning | Compile-time process that chooses ownership partitions before runtime preparation. |
| Ownership partition | A group of graph nodes assigned to a backend as one planned partition. |
| Accelerator partition | An ownership partition for Metal, CUDA, or another accelerator backend. |
| CPU natural partition | A CPU-owned group of nodes that can later be split into unit kernels and fused units. |
| Partition execution planning | The phase that turns an ownership partition into execution units. |
| Execution unit | A concrete unit inside a partition, for example one CPU matmul kernel or one fused elementwise chain. |
| Memory planning | Compile-time lifetime and reusable-buffer planning across graph and partitions. |
| Runtime selection | Prepare-time choice of the executable backend path using `RuntimeConfig` and availability checks. |
| CPU native storage partition | CPU execution partition that keeps selected values in `MemorySegment` storage for provider-backed routes or explicit native diagnostics. |

The distinction between ownership partition and execution unit is critical:

```text
ownership partition:
  CPU owns nodes [matmul, add, relu, sum]

execution units inside that partition:
  unit 1: matmul kernel
  unit 2: fused add+relu loop
  unit 3: sum reduction kernel
```

One partition does not imply one runtime kernel.

## CPU Native Storage Partitions

CPU native storage is planned as a storage route inside the CPU backend, not as another backend family.
`CpuPartitionLowerer` may lower a CPU-owned subpartition to a native-storage partition when runtime policy allows it.
That partition still executes through CPU-owned prepared artifacts and existing operation-family kernels.

The current selection policy is intentionally conservative:

| Runtime policy | Native-storage behavior |
|---|---|
| `CPU_ARRAY` | No native CPU storage partition is selected for compute. |
...
The native partition trace exposes selected/rejected decisions, provider/local/view nodes, storage contracts, physical
kernel families, layout materialization reasons, and `nativeCpuPartitionAutoEligible`. This evidence is the replacement
...
CompileConfig compile = CompileConfig.training()
        ...
        .withPartitionExecution(PartitionExecutionConfig.trainingDefaults())
...
| Backend planning | `BackendPlanningConfig` | CPU-only, explicit accelerator, or automatic accelerator partition discovery. |
| Partition execution planning | `PartitionExecutionConfig` | Fusion and execution-unit planning inside owned partitions. |
| Memory planning | `MemoryPlanningConfig` | Lifetimes, reusable runtime slots, partition value flow, partition bindings, handoff requirements, runtime binding policy, and summary metrics. |
...
- `src/main/java/config/compile/PartitionOwnershipPlannerStrategy.java`
...
| Field | Meaning |
|---|---|
| `discoveryMode` | How accelerator partitions may be discovered: `CPU_ONLY`, `EXPLICIT`, or `AUTO`. |
...
| `ownershipPlanner` | Partition planner strategy, currently mapped to anchor or scored partition planners. |
| `search` | Search limits and score weights. It does not select backend target by itself. |
| `cpuPartitions` | CPU natural-partition policy and width limits. |
| `cost` | Planning cost profile, including Metal transfer model for scored accelerator planning. |

### Discovery Modes

`CPU_ONLY` means no accelerator ownership partitions are planned. CPU natural partitions may still be planned if CPU partition policy is enabled.
...
`EXPLICIT` means only nodes that already carry explicit accelerator backend intent can seed accelerator planning. The planner can still include legal supporting nodes required for a valid closed partition when the target backend partition capability allows it.
...
`AUTO` means the planner may discover accelerator partitions from CPU-owned graph structure according to target legality and cost policy.
...
EXPLICIT:
  no GPU partition unless some node already carries GPU intent

AUTO:
  planner may consider matmul/add/relu as a GPU partition if legal and profitable
```
...
| `BackendPlanningConfig.autoAccelerator()` | Auto-discover accelerator partitions when legal/profitable, failure optional. |
| `BackendPlanningConfig.requireAnyAcceleratorPartition()` | Require at least one accelerator partition from auto planning. |
| `BackendPlanningConfig.requireEachAcceleratorTarget()` | Require a partition for each configured accelerator target. |
| `BackendPlanningConfig.requireAllExplicitIntents()` | Every explicit accelerator intent must be represented by a legal planned accelerator partition. |
...
- `src/main/java/planning/backend/BackendPlanningJobResolver.java`
- `src/main/java/planning/backend/BackendPlanningService.java`
...
- which partition planner strategy is used
- whether a CPU natural-partition job is added
...
jobs:
  ...
  2. CPU natural-partition job
     reason = cpu-natural-partition
```

For `EXPLICIT`, the GPU job is emitted only if the graph contains explicit `GPU_METAL` or `GPU_CUDA` intent.

## Partition Planning Strategies

The public compile strategy enum is `PartitionOwnershipPlannerStrategy`.

| Strategy | Practical meaning |
|---|---|
| `ANCHOR` | Build partitions from backend-capable anchors and expand through legal supporting structure. |
| `SCORED` | Search candidate partitions and score them using structural and transfer-cost weights. |

The backend planning resolver maps those public strategies to internal partition planners. The internal word "partition" still appears in implementation classes such as `PartitionPlan` and `ScoredCandidatePartitionPlanner`, but in architecture language it means "planned ownership partition", not "optimizer stage".

## CPU Natural Partitions

CPU partition planning is not a GPU fallback afterthought. It gives the later partition optimizer a coherent CPU-owned graph area.

Source:

- `src/main/java/config/optimizer/CpuPartitionConfig.java`
- `src/main/java/planning/partition/CpuNaturalPartitionPlanner.java`
- `src/main/java/planning/partition/execution/CpuPartitionExecutionPlanningPolicy.java`

`CpuPartitionConfig.defaults()` enables natural CPU partitions with a maximum partition size. Current default `maxPartitionNodes` is `64`.
...
The CPU natural-partition planner may group nodes `[3, 4, 5, 6]` into one CPU execution partition. That is a planning artifact. The partition optimizer may still split it into multiple execution units:
...
Accelerator partitions are stricter than CPU natural partitions. They must satisfy target legality.
...
- Are partition inputs and outputs legal for buffer binding or fallback paths?
- Does the partition require a supporting producer, such as a `PERMUTE`, to be included?
...
Partition optimization happens after ownership is decided.

Source:

- `src/main/java/config/compile/PartitionExecutionConfig.java`
- `src/main/java/planning/partition/execution/PartitionExecutionPlanner.java`
- `src/main/java/planning/partition/execution/CpuPartitionExecutionPlanningPolicy.java`
- `src/main/java/planning/value/GraphValueRef.java`

Partition optimization answers:
...
- Which values must be materialized as partition outputs or handoff values?

Term explanations:

- Materialized value: a value that exists in runtime storage after a unit runs.
- Continuation value: a value that can be forwarded inside a partition without publishing to a public tensor.
...
Memory planning is compile-time lifetime analysis. It decides which temporary runtime storage slots may be reused and where partition handoff values must be materialized. `MemoryPlanner` is the public entry point; the package-local planners under `planning.memory` own tensor lifetimes, reusable interval filtering, slot allocation, partition value flow, partition bindings, handoff requirements, runtime binding policy, and summary metrics.

Source:

- `src/main/java/config/compile/MemoryPlanningConfig.java`
- `src/main/java/planning/memory/MemoryPlanner.java`
- `src/main/java/planning/memory/MemoryPlan.java`
- `src/main/java/planning/memory/TensorLifetimePlanner.java`
- `src/main/java/planning/memory/PartitionExecutionValueFlowPlanner.java`
- `src/main/java/planning/memory/PartitionBindingAllocator.java`
...
If `t1` is consumed only by `t2` and no public tensor needs `t1`, memory planning can avoid treating `t1` as an independently published long-lived value. If a later partition boundary needs `t2`, partition value flow records the node-id to graph-value mapping, partition binding allocation chooses reusable partition slots, and handoff planning creates a requirement for `t2`.
...
Here `CompileConfig.trainingAutoAccelerator()` allows compile-time Metal partition discovery. `RuntimeConfig.trainingDefaults()` still decides whether the Metal backend is enabled and available at prepare/runtime.
...
- `src/main/java/runtime/execution/PublicationPolicy.java`
- `src/main/java/runtime/execution/PreparedExecution.java`
...
Publication is not backend planning. Disabling publication can avoid public tensor synchronization costs, including device-to-CPU copies, but it does not change which backend owns a partition.
...
- CPU partition policy
- CPU fusion policy
- backend planning mode
- partition planner strategy
...
6. Check run trace fallback reason if a planned partition executed on CPU.

When a CPU path is slower:

1. Check whether the compile policy changed graph shape or partition splits.
