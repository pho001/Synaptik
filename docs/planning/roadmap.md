...
| 11 | [`modules/engine`](modules/engine/master-plan.md) | Ready 0008A after Complete 0008 | The post-CPU-0010J audit found that Engine already owns representative inputs, publication descriptors, canonical host copying, result cleanup, and fallback. | Implement the package-private exact complete-plan correctness oracle required before tools/tuning 0002; add no public API, timing, cache, candidate interpretation, or fallback change. |
...
| 22 | [`tools/tuning`](tools/tuning/master-plan.md) | Blocked after 0001 | CPU 0010J supplies the first complete-plan producer, and the fresh audit resolves the generic contract, but Ready Engine 0008A must implement its correctness oracle first. | After 0008A, freshly audit and specify task 0002 with checked `N * (1 + W + S)` budgeting, opaque match/mismatch collaboration, SESSION-aware compact records/cache, deterministic selection, and separate rich evidence; task 0003 remains downstream. |
...
Engine composition. The post-CPU-0010J audit keeps tuning 0002 `Blocked` without a specification
and selects Ready
...
as its single owning prerequisite; 0003 remains `Draft`.
Detailed
...
Compiler 0006B3 Engine-facing complete compile integration port (Complete)
  ...
  -> Engine 0008A representative complete-plan correctness oracle (Ready prerequisite interleave)
  -> tools/tuning 0002 (Blocked; no detailed specification pending post-0008A audit)
```
...
as the now-Complete complete-plan producer prerequisite. The completed post-0010J audit resolves
the generic Phase-2 contract but keeps task 0002 Blocked without a specification because Engine's
current representative session cannot compare publications. Ready
[Engine 0008A](modules/engine/tasks/0008a-representative-complete-plan-correctness-oracle.md)
is the smallest owning prerequisite; 0002 requires a fresh post-0008A audit.
...
prerequisite. The completed fresh audit keeps tools/tuning 0002 Blocked, selects Ready Engine
0008A for the missing exact correctness oracle, and still creates no 0002 specification.
...
the local phase; Complete CPU 0010J now exposes these retained alternatives. The fresh audit
selects Ready Engine 0008A before blocked Tuning 0002 can specify compatible end-to-end
promotion. The first full corrective CPU rerun is
...
tuning 0002 may consume them only after Ready Engine 0008A implements the audited exact
correctness oracle, a fresh post-implementation audit creates its specification, and model
autotuning is explicitly requested before Runtime. The tuning workflow
...
[NN 0025A](extensions/nn/tasks/0025a-dimensional-convolution-user-capability-checkpoint.md)
is also `Complete`. CPU 0010J is Complete; tools/tuning 0002 remains Blocked on Ready Engine
0008A and a fresh post-implementation audit. Compiler 0006C and NN 0021B–0024 remain Draft.
Family tasks
