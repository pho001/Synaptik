...
| 10 | [`backends/cpu`](backends/cpu/master-plan.md) | In progress (CPU 0010C Complete; CPU 0010D Draft) | Model, config, planning, runtime, prepare, backend-contract, trace, and OpenBLAS contracts are ready. | CPU is a conforming reference backend for the selected capability set. |
...
The active detailed planning frontier is
...
which remains `Draft`. CPU 0010E remains a master-plan-only `Draft` row behind it.
