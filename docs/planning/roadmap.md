...
| 7 | [`modules/compiler`](modules/compiler/master-plan.md) | Complete through 0006B7 | The Metal 0002 review identified one final Compiler-owned logical-descriptor prerequisite after the completed Engine interleave. | Compiler 0006B7's exact NEG output/direct-splat-input closure is complete; 0006C and 0007 remain independent Draft side branches. |
...
| 22 | [`tools/tuning`](tools/tuning/master-plan.md) | Complete through 0003 | Both compact schema-1 artifacts and their separate rich result evidence are stable after tasks 0001–0002. | [Task 0003](tools/tuning/tasks/0003-read-only-cache-plan-and-evidence-inspection.md) completed bounded Path/byte inspection, redacted provenance and selected-candidate summaries, typed invalid/mismatch reasons, and evidence summaries without decoder, execution, or mutation. |
...
Detailed
[OpenBLAS provider 0004 optional direct BFLOAT16-output GEMM capability](backends/openblas-provider/tasks/0004-optional-direct-bfloat16-output-gemm-capability.md)
is `Blocked` and deferred as an optional capability side branch: pinned OpenBLAS v0.3.31 fails
both the exported-C-ABI and one-final-BFLOAT16-narrowing gates. CPU 0010D1 is the corresponding
blocked/deferred optional downstream row. Neither row is deleted, neither changes Model or
portable CPU BFLOAT16 support, and neither gates existing FLOAT32/FLOAT64 OpenBLAS work.

Detailed
[CPU 0010E FLOAT32/FLOAT64 OpenBLAS tuning candidates and compatible decisions](backends/cpu/tasks/0010e-float32-float64-openblas-tuning-candidates-and-compatible-decisions.md)
is `Complete` after completed
[CPU 0010D installed OpenBLAS qualification and target fingerprinting](backends/cpu/tasks/0010d-installed-openblas-qualification-and-target-fingerprinting.md).
Detailed
[Prepare 0004 opaque backend candidate-batch and selected-decision handoff](modules/prepare/tasks/0004-opaque-backend-candidate-batch-and-selected-decision-handoff.md)
is `Complete`. The acyclic mainline remains CPU 0010E ->
Prepare 0004 -> tools/tuning 0001: CPU defines the typed candidates and canonical compatibility
facts it owns; Prepare transports candidate batches and decisions opaquely without measurement;
tuning finally owns measurement, selection, persistence, corruption/incompatibility rejection,
and rich evidence. CPU 0010E and Prepare 0004 are `Complete`. Detailed
[tools/tuning 0001 exact/default model-guided workload tuning and reusable cache](tools/tuning/tasks/0001-exact-default-model-guided-workload-tuning-and-reusable-cache.md)
...
is `Complete` with bounded read-only inspection of both stable formats and detached redacted
summaries of result-evidence models.
Detailed
[Config 0006A](modules/config/tasks/0006a-model-autotuning-request-configuration.md) and
[Config 0006B](modules/config/tasks/0006b-complete-plan-autotuning-request-configuration.md) are
the `Complete` Config request facade sequence delivered through the two recorded non-overlapping
staged ordering exceptions. Completed tuning 0002, CPU 0010J, and Engine 0008A now support detailed
...
The next operational-lifecycle sequence is:
...
is `Complete`. The actual complete Compiler implementation entry remains package-private and
takes package-private constant ingress; the completed same-package public
...
Engine 0006 is Complete; none enables a zero-node schedule. Engine 0005A is independent of this
...
`output.execute()`; explicit-target one-shot scalar-objective backward convenience is complete in
0006 without adding Tensor mutation or no-argument backward.
...
is `Complete` after implementation context `01a0b11d-cc89-7590-8836-0555b05e7001` and clean
documentation context `01a0b126-f60b-7a11-8d7c-621e532952f0`. It selects one explicit-target
`Engine.backward(...)` call, Compiler's absent scalar
...
implementation uses the completed
...
is `Complete`. The initial audit blocked tools/tuning 0002 and selected detailed
[CPU 0010J](backends/cpu/tasks/0010j-supported-complete-plan-candidate-and-decision-producer.md)
as the now-Complete complete-plan producer prerequisite. The completed post-0010J audit resolved
the generic Phase-2 contract but kept task 0002 Blocked without a specification because Engine's
...
0002 and detailed 0003 read-only cache, plan, and evidence inspection are Complete.
...
and establishes the portable grouped NCHW Conv2d execution foundation. CPU 0008A, CPU 0008B, and
CPU 0008C are Complete. Detailed
[CPU 0008D bounded fusion profitability and typed decision facts](backends/cpu/tasks/0008d-bounded-fusion-profitability-and-typed-decision-facts.md)
is Complete after its clean documentation gate; detailed
[CPU 0008E bounded multi-input materialization and representation reuse](backends/cpu/tasks/0008e-bounded-multi-input-materialization-and-representation-reuse.md)
is Complete after its corrective implementation and final documentation pass. The older same-instruction two-copy
failure remains `CO_CONSUMED_PAIR`. The newly retained honest production-selectable FLOAT64 heap
one-copy row passes generated/optimal-Java at `1.000326680` but fails complete-plan
selected/direct at `2.605250934` with all five samples retained. Every materialized form therefore
remains a complete executable candidate, but ordinary preparation selects CPU 0008D's direct
...
candidates explicitly and did not reopen production selection. That implementation now passes
115 focused tests and the final 544-test/104-suite CPU run with 3 expected skips and no failures
or errors. Final clean documentation context `01a04317-b784-76e3-a93b-ff35106284b9` completed the
targeted CPU-guide/Javadoc and three-entry glossary correction. Documentation context
`01a04303-9552-72a0-a5dc-c95d20413779`
stopped without edits because the previous task scope did not authorize the stale guide; the
corrected plan authorized it. Replacement documentation context
`01a0430b-de37-7de3-bfa1-9b21f2cd21d3` then stopped without edits because its audit demonstrated
...
0008E1 is Complete; detailed
[CPU 0008F](backends/cpu/tasks/0008f-portable-matmul-execution-and-bounded-linear-epilogues.md)
is Complete with its retained semantic, Class-File, and performance evidence.
...
CPU checkpoint all passed. CPU 0008I is `Complete` with an explicit project-decision validation
...
detailed [CPU 0008K](backends/cpu/tasks/0008k-cross-type-cast-execution.md) and
[CPU 0008L](backends/cpu/tasks/0008l-pointwise-simd-mask-output-closure.md) are `Complete`.
CPU 0008L uses a bounded fixed-`ByteVector` bridge for dense FLOAT32/FLOAT64 mask publication and
`WHERE` reload while preserving virtual masks, scalar tails, and selective schema-61 identity.
Its 72-Class-File structural gate, all 540 generated/optimal-clean-Java sample pairs, and final
715-test CPU rerun passed. Detailed
[CPU 0008M](backends/cpu/tasks/0008m-vector-mse-none.md) is `Complete`; detailed
...
passed `V/S` at `0.265215012x` but failed `V/D` at `1.775769426x > 1.15x`.
[CPU 0008N1](backends/cpu/tasks/0008n1-generated-conv-nested-width-block-loop-dataflow-parity.md)
implemented production schema-63 direct dense FLOAT32/FLOAT64 Conv2d/Conv3d output-width vector
routes. Stage B passed 48 semantic rows, 24 structural dossiers, and all 16 five-fork performance
...
five-fork acceptance is not passed. [CPU 0008P](backends/cpu/tasks/0008p-deterministic-partial-reduction-parallelism.md)
is Complete as a bounded fail-closed result: its generated INT32/INT64 modular partial/combine
architecture is implemented, but its only authorized profitability root failed before sealing or
timing, so all production selection remains `KEEP_WHOLE_CELL`. Model 0018V has no current
floating conformance tolerance and grants no floating reassociation.
Broad validation remains CPU 0009/CI.
...
accumulation order remain unchanged. All 24 structural dossiers, 540 performance pairs, scalar
preservation, focused/full CPU validation, and corrected preparer Javadoc passed.
...
reduction numerical spike, then partial-reduction parallelism. CPU 0008Q is Complete as a strict,
executable BFLOAT16 scalar-MUL represented-immediate mechanism only, not the complete scalar/clamp
matrix; its independent documentation pass is complete. Independent CPU 0008Q1 execution exposed
a Synaptik helper call in generated vector positive-one scalar power, with reciprocal power sharing
...
CPU 0009 is Complete: its canonical 17,636-row inventory (17,463 generated plus 173 exact
rejections) has complete joined semantic-fixture closure. Final 0009G reconciles all 120 live
operation forms to the readable ledger and checks dense/general layout presence plus the four
actually selected strategies where meaningful; it does not promote a universal clean-Java
structural oracle or representative benchmark. CPU 0009A is Complete: all 256
...
scatter, and 20 random); parent [CPU 0009D](backends/cpu/tasks/0009d-aggregate-scan-ordering-fold-structural-oracle-closure.md) is Complete. Its D1--D4 children retained generated routes because a direct replacement would add greater implementation and verification cost, with no performance claim. [CPU 0009E1 partial integral reduction](backends/cpu/tasks/0009e1-partial-integral-reduction-direct-java-migration.md), [CPU 0009E1A masked reduction](backends/cpu/tasks/0009e1a-masked-reduction-route-decision.md), [E1B1 log-sum-exp](backends/cpu/tasks/0009e1b1-log-sum-exp-route-decision.md), [E1B2 statistical reductions](backends/cpu/tasks/0009e1b2-statistical-reduction-route-decision.md), [E1B3 norms](backends/cpu/tasks/0009e1b3-norm-reduction-route-decision.md), [E1C softmax-style](backends/cpu/tasks/0009e1c-softmax-style-reduction-route-decision.md), [E2 normalization](backends/cpu/tasks/0009e2-normalization-route-decision-and-possible-direct-java-migration.md), and [E3 loss](backends/cpu/tasks/0009e3-loss-route-decision-and-possible-direct-java-migration.md) are Complete retained generated routes. E3 independently retains complete MSE, dense categorical, and index categorical matrices because direct Java would add implementation, binding, and retirement proof; this is not a performance claim. Parent CPU 0009, 0009F, and F1--F3 are Complete; [0009G](backends/cpu/tasks/0009g-final-support-correctness-hygiene-and-inventory-checkpoint.md) closed truthful accounting for all 120 live forms without a universal strategy, structural, or performance claim. Existing performance facts remain reported, but no new benchmark gate blocks correctness closure.
Current correction: E1B, E1C, E2, and E3 are Complete retained generated routes, with no performance claim; parent CPU 0009 is Complete, while parent E remains a master-plan summary and parent 0009F and F1--F3 are Complete. Detailed [CPU 0009G final support, correctness, hygiene, and inventory checkpoint](backends/cpu/tasks/0009g-final-support-correctness-hygiene-and-inventory-checkpoint.md) is Complete.
...
and tools/tuning 0002 and detailed tuning 0003 read-only inspection are Complete.
...
dimensional-convolution closure depend on later general fusion or materialization facilities
without a concrete documented need.

This earlier convolution-specific no-shared-task conclusion remains historical. The new
user-authorized Prepare 0003A interleave addresses a separately demonstrated general boundary:
one immutable partition-local DAG projection prevents complete cross-backend graph exposure and
removes the need for concrete backends to reconstruct exact structural occurrences independently.
Planning still selects only backend ownership, and Runtime still executes prepared schedules
without interpreting graph topology.
...
The completed current Metal implementation is documented in
...
is `Complete` and remains the native device/queue and run-owned storage foundation. Detailed
[Metal 0002 MPSGraph prepared execution route](backends/metal/tasks/0002-mpsgraph-prepared-execution-route.md)
is `Complete` after detailed
[Compiler 0006B7 final NEG logical-layout closure](modules/compiler/tasks/0006b7-final-neg-logical-layout-closure.md)
completed its mandatory prerequisite. The first truthful executable route is
every non-empty maximal Metal-owned partition composed solely of individually supported positive-
static-shape resolved dense-contiguous `FLOAT32` unary `NEG` occurrences. One reusable executable
lowers chains, independent nodes, fan-out, repeated consumption, internal values, multiple
boundaries, and differing valid shapes without changing occurrence capability or partitioning.
Because occurrence capability queries carry descriptors but no constant provenance, Metal cannot
reject an otherwise eligible NEG whose direct feed is a compile-time splat. Compiler 0006B7 first
closes only fully static still-unresolved exact NEG outputs and directly consumed splat inputs;
Model and generic elementwise inference remain unresolved, and scalar/zero-extent logical closure
does not relax Metal's stricter positive rank-`1..16` route.
Runtime 0016 gives `PreparedExecution` the owned-resource and close lifecycle, and Prepare 0006
returns and transfers finalizer resources transactionally. ADR
0013 resolves the complete architecture: `PreparedExecution` owns a private identity-unique
resource aggregate, active synchronous runs lease it, and close rejects new runs without waiting.
...
Runtime 0016 persistent prepared-resource lifecycle (Complete; detailed)
  ...
  -> Compiler 0006B7 final NEG logical-layout closure (Complete; detailed)
  -> Metal 0002 MPSGraph prepared execution route (Complete; detailed)
```
...
or Prepare ownership. The completed Metal planning audit found no shared-contract or architecture
gap: installed SDK 27.0 headers provide fixed-shape compilation, supplied `MTLBuffer` tensor-data
input/result binding, synchronous reusable execution, and completion-error reporting. The exact
temporary probe compiled in the sandbox but there observed `NO_DEVICE`; the coordinator then ran
that binary outside the sandbox, where it exited `0` and printed
`PASS executable_reused=2 direct_output_buffer=1 completion_errors=0 results=1`. This is bounded
feasibility evidence, not repository integration or failure-mapping proof. The completed repository
route now provides that integration evidence. ABI version `2` has exactly ten exports: the seven
foundation symbols plus three executable functions, with statuses `0..11`. Analysis declares all
boundary buffers and one address workspace; finalization compiles one persistent resource; each
run materializes eligible splats once during cold run-state creation, owns fresh outputs and
pre-marshalled native pointer storage, and keeps hot execution to one synchronous downcall into
supplied destinations.

Evidence composition is explicit. The real-device caller-input route passes through public
`GraphCompilationPort`, shared `GraphPreparation`, Runtime, and MPSGraph. Backend-local typed
`PrepareContext` tests cover positive-rank splats through analysis/finalization/schedule/Runtime,
while existing shared Prepare tests prove
`ConstantSource -> PrepareContext.constants() -> InitializedBuffer`. No public-port positive-rank
splat test is claimed because that port cannot express an explicit positive-rank forward
constant. Deterministic fake/native-seam coverage is the accepted status/error matrix.

The ordinary Metal suite passed 35 total tests with three expected opt-in skips; focused execution
passed 20 total tests with two expected opt-in skips. The native build, exact ten-symbol and
Foundation/Metal/MetalPerformanceShadersGraph link inspections, both exact real-device tests
outside the sandbox, focused backend conformance and architecture tests, source/`javap` hot-path
...
and detailed [CPU 0008K Cross-type CAST execution](backends/cpu/tasks/0008k-cross-type-cast-execution.md)
...
fork-0 `KEEP_SCALAR` evidence but no completed five-fork acceptance; CPU 0008P is Complete with
its modular partial/combine implementation fail-closed at `KEEP_WHOLE_CELL`, not floating
reassociation.
...
gradient closure. Detailed CPU 0008, CPU 0008A, CPU 0008B, and CPU 0008C are Complete. Detailed
[CPU 0008D](backends/cpu/tasks/0008d-bounded-fusion-profitability-and-typed-decision-facts.md)
is Complete and records the final evidence. Detailed
...
full-K generated realizations, disjoint output work units, exact bounded linear epilogues or safe
split, and direct plus candidate-only one-input materializations. Its implementation context
passed 17 focused suites/182 tests, the sole 109-suite/576-test CPU checkpoint, CPU Javadoc,
Class-File/forbidden-reference inspection, and five immutable performance forks; the worst
individual generated/direct kernel ratio was `1.002573` against the required `<= 1.15x` gate.
The 212-file attempt-16 manifest verifies with digest
`a88806a9118c1a967ecc77eaf6da9582d15afa959e8368348a0d2ba2a47d4b61`. Materialization
companions remain evidence-backed candidates rather than automatic default policy. Clean
documentation context `/root` finalized CPU Javadocs, the backend guide, the existing MATMUL
glossary entry, and planning status without changing executable Java. Architecture, dependencies,
shared/public APIs, Gradle, conformance, integration, and native routes remain unchanged.
...
performance evidence, fusion policy, hot code, or Runtime behavior. Detailed Complete CPU 0008F
follows 0008E1 and now owns bounded portable MATMUL execution. Detailed Prepare 0004 retains its
completed 0003A dependency, is `Complete` without renumbering, and follows CPU 0010E.
...
documentation context `01a043d7-113c-7ee2-8257-42678c1a7be4` finalized the public and package
Javadocs, API status, focused boundary explanation, backend guide, glossary, and planning
evidence. The task-required shared checkpoint ran exactly once and passed 364 suites and 2,491
tests with the same 3 expected skips and no failures or errors, including 4 architecture-test
suites and 6 architecture tests. The final scope is 17 authorized paths, with exactly one CPU
test path and no CPU production, Gradle, architecture-test source, backend-conformance, or
...
The original Prepare 0001–0003 milestone and user-authorized interleaved Prepare 0003A are
Complete. OpenBLAS provider tasks 0001–0003 and their original milestone remain Complete; optional
provider task 0004 reopens the project area without changing that completed history.
...
their respective prerequisites. Detailed Complete
[CPU 0007B](backends/cpu/tasks/0007b-portable-arg-extrema-coverage.md) owns arg extrema, while
detailed Complete [CPU 0007C](backends/cpu/tasks/0007c-portable-masked-reduction-coverage.md) owns
masked reductions. Detailed Complete
[CPU 0007D](backends/cpu/tasks/0007d-portable-logarithmic-statistical-and-norm-reduction-coverage.md)
owns advanced logarithmic/statistical/norm reductions. Detailed Complete
[CPU 0007E](backends/cpu/tasks/0007e-portable-stable-softmax-and-log-softmax-coverage.md) owns
stable softmax/log-softmax. Complete
[CPU 0007F](backends/cpu/tasks/0007f-portable-layer-and-rms-normalization-coverage.md) owns the
cohesive trailing-slice Layer/RMS boundary; Complete
[CPU 0007F1](backends/cpu/tasks/0007f1-portable-batch-normalization-inference-coverage.md) owns
coordinatewise batch inference with channel-hoisted scalar work and deterministic zero-workspace
...
`Complete`. Detailed
[CPU 0007B](backends/cpu/tasks/0007b-portable-arg-extrema-coverage.md) is `Complete`. Detailed
[CPU 0007C](backends/cpu/tasks/0007c-portable-masked-reduction-coverage.md) is `Complete`. Detailed
[CPU 0007D](backends/cpu/tasks/0007d-portable-logarithmic-statistical-and-norm-reduction-coverage.md)
and detailed CPU 0007E and CPU 0007F are `Complete`, while detailed CPU 0007F1 is `Complete`;
detailed CPU 0007F2 is `Complete`, and later work remains Draft without detailed specifications. A2's corrected
37-path scope
permits the A1O ledger
...
0007B retained evidence reports `VERIFIED,6`, 384 CPU tests with zero failures or errors and one
expected skip, and all 30 fork plus all six aggregate generated/direct ratios at `<= 1.15x`; the
maximum observed fork ratio is `1.038305072x`. Its 40-path final scope advances the CPU-private
generator to schema 44 with exactly three new production types and no public API, architecture,
dependency, build, conformance, or integration change. CPU 0007C retained evidence verifies six
FLOAT64/FLOAT32/BFLOAT16 SUM/MEAN classes, mask-before-data ordering, exact selected counts, and
five isolated generated-versus-optimal-direct-Java forks whose worst ratio is `0.980505473x`.
The final uncached CPU suite reports 398 tests, one expected skip, and no failures or errors. Its
42-path final scope advances current compatibility to schema 45 with exactly three new CPU-private
production types and no public/shared/build/architecture/conformance/integration change. Schema-42
ledger and performance material remains explicitly historical. CPU 0007D retained evidence
contains fifteen generated targets across five kinds and three floating representations plus
three unchanged-family controls. All passed five isolated optimal-direct-Java forks at
`<= 1.15x`; the worst fork ratio is `1.099406784x` and the worst aggregate ratio is
`1.087934030x`, both FLOAT64 log-sum-exp. The final uncached CPU suite reports 416 tests, one
expected opt-in skip, and no failures or errors. Complete Class-File/member inspection found no
forbidden hit. Its exact 45-path scope comprises 27 production/Javadoc, 13 test, and five named
documentation/planning paths, with exactly six new CPU-private production types and no public/
shared/build/architecture/conformance/integration change. Current compatibility is schema 46;
schema-42 retained evidence remains historical. CPU 0007A1D remains Review needed.
CPU 0007E retained evidence covers eight first-class SOFTMAX/LOG_SOFTMAX targets and three
unchanged-family controls across five sequential isolated Java 26.0.1 forks. All 55 per-fork and
11 aggregate generated/optimal-direct-Java ratios passed `<= 1.15x`; the worst fork was
`1.119471916x` and the worst aggregate was `1.113919290x`, both BFLOAT16 LOG_SOFTMAX heap. The
post-fix uncached CPU suite reports 70 suites/428 tests, zero failures/errors, and one expected
skip; CPU Javadoc passed with two incubator warnings. Generated SOFTMAX contains no `Math.log`,
LOG_SOFTMAX retains its required logarithm, and the complete member audit found no forbidden hit.
The 163-file retained bundle verifies all 162 checksum entries and contains all 70 XML reports.
Its final 47-path scope comprises 29 production/Javadoc, 13 test, and five documentation/planning
paths, including exactly five new CPU-private production types and schema 47. It changes no
public/shared/build/architecture/conformance/integration boundary. CPU 0007E is Complete, CPU
0007A1D remains Review needed, and detailed CPU 0007F is Complete. Detailed CPU 0007F1 is Complete;
detailed CPU 0007F2 is Complete, and later tasks remain Draft without detailed specifications.

CPU 0007F retained evidence covers fourteen first-class Layer/RMS targets and three unchanged-
family controls across five isolated forks. All 85 per-fork and 17 aggregate generated/optimal-
clean-Java ratios passed `<= 1.15x`; the worst fork was `1.136183168x` and the worst aggregate was
`1.113266704x`, both BFLOAT16 RMS. The final uncached CPU suite passed 76 suites/449 tests and the
focused post-fix run passed 10 suites/119 tests, both with zero failures/errors. All 17 generated
classes passed complete Class-File/member scans, and the 241-file retained manifest verifies with
digest `a7ca999336d73dbf2fee3d2414ff31f0339a5048ec7e2b5ab804b1c5186829c9`. Its final 35-path
scope comprises 16 production/Javadoc, 14 test, and exactly five documentation/planning paths,
...
and CPU 0008F are Complete, and later tasks remain Draft.
...
deterministic channel/non-channel ranges, heap/segment/mixed carriers, and zero workspace.
Focused validation passed 112 tests and the final uncached CPU XML reports 462 tests with zero
failures, zero errors, and two expected skips. Five accepted frozen forks passed every
batch/control `<= 1.15x` gate; the 281-file retained bundle verifies with digest
...
detailed CPU 0008E1 and CPU 0008F are Complete.
...
scalar or parallel-scalar range, computes biased saved variance and separately divided unbiased
running variance, and publishes explicit next and saved statistics without hidden state. The
focused evidence passed 10 suites/114 tests; the implementation context's final uncached CPU
suite passed 86 suites/472 tests with two expected skips and no failures or errors. Eight
deterministic generated classes passed complete `javap`, member, forbidden-reference, and
operation-count scans. Five accepted isolated Java 26.0.1 fixed-heap forks passed all 60 per-fork
and 12 aggregate ratios at `<= 1.15x`; the worst fork and aggregate were `1.123421082x` and
...
`d1138b75924cea2b1bbce6ba127213eb2181de7156884b857ce1e475b9b95edb`. The worktree has 47 paths:
...
Complete; Prepare 0003A, detailed CPU 0008E1, and CPU 0008F are Complete, and later tasks remain
Draft. The
Conv1d/Conv3d-before-general-DAG ordering correction
remains intact.

The isolated `M-CONCAT` A1G
...
are `Complete`. Detailed CPU 0007A2, detailed CPU 0007B, and detailed CPU 0007C are `Complete`.
Detailed CPU 0007D, detailed CPU 0007E, and detailed CPU 0007F are `Complete`; detailed CPU 0007F1
is `Complete`; detailed CPU 0007F2 is `Complete`, and later work remains Draft without detailed specifications.
...
CPU 0007A1O, CPU 0007A2, detailed CPU 0007B, and detailed CPU 0007C are `Complete`. CPU 0007D is
detailed and `Complete`; detailed CPU 0007E and detailed CPU 0007F are `Complete`; detailed CPU
0007F1 is `Complete`; detailed CPU 0007F2 is `Complete`, and later work remains Draft without detailed specifications.
...
`KEEP_SCALAR` evidence but no completed five-fork acceptance, and CPU 0008P is Complete with its
modular partial/combine implementation fail-closed at `KEEP_WHOLE_CELL`, not floating reassociation,
and CPU 0008Q is [`Complete`](backends/cpu/tasks/0008q-scalar-immediate-clamp-generated-equivalence.md) as a strict two-fixture BFLOAT16 scalar-MUL mechanism, CPU 0008Q1A is [`Complete`](backends/cpu/tasks/0008q1a-vector-scalar-power-hot-path-self-containment.md), CPU 0008Q1 is [`Complete`](backends/cpu/tasks/0008q1-finite-scalar-immediate-clamp-matrix.md), and CPU 0009 is [`Complete`](backends/cpu/tasks/0009-portable-generated-coverage-closure-checkpoint.md): completed 0009A--G remain historical evidence. CPU 0009E1, E1A, E1B1, E1B2, E1B3, E1C, E2, and E3 are Complete retained generated routes. E3 independently retains MSE, dense categorical, and index categorical generated routes because direct replacement would increase implementation and verification cost, without a performance claim. Final 0009G closes all 120 live-form accounting with meaningful selected-strategy facts, not universal strategy, structural, or performance proof. The retained loss fork-0 failure remains NON_PASSING historical performance debt. [CPU 0010](backends/cpu/tasks/0010-narrow-openblas-blas-compatible-native-route.md) is `Complete`. [CPU 0010A automatic OpenBLAS discovery and internal composition foundation](backends/cpu/tasks/0010a-automatic-openblas-discovery-and-internal-composition-foundation.md) is `Complete`. Detailed [CPU 0010B bounded OpenBLAS MATMUL representation expansion](backends/cpu/tasks/0010b-bounded-openblas-matmul-representation-expansion.md), [CPU 0010C coordinated OpenBLAS thread candidates and shared CPU thread budget](backends/cpu/tasks/0010c-coordinated-openblas-thread-candidates-and-shared-cpu-thread-budget.md), and [CPU 0010D installed OpenBLAS qualification and target fingerprinting](backends/cpu/tasks/0010d-installed-openblas-qualification-and-target-fingerprinting.md) are `Complete`. Optional OpenBLAS provider 0004 and dependent CPU 0010D1 are blocked/deferred side-branch work after the direct-BFLOAT16 proof failed; neither gates FLOAT32/FLOAT64. Detailed [CPU 0010E](backends/cpu/tasks/0010e-float32-float64-openblas-tuning-candidates-and-compatible-decisions.md) is `Complete`; detailed Prepare 0004 is `Complete`; detailed tools/tuning 0001 is Complete in the CPU 0010E -> Prepare 0004 -> tools/tuning 0001 mainline. CPU 0011 remains independently `Blocked` because the repository has no concrete Intel CPU use case or supported oneMKL BLAS/VML ABI evidence. CPU 0012–0015, narrowed cross-route CPU 0016, and CPU 0017 remain `Draft` rather than advancing out of order.

Current correction: parent CPU 0009D is Complete. [CPU 0009D1 aggregate](backends/cpu/tasks/0009d1-aggregate-structural-oracle-closure.md), [CPU 0009D2 scan](backends/cpu/tasks/0009d2-scan-route-decision-and-possible-direct-java-migration.md), [D3 ordering](backends/cpu/tasks/0009d3-ordering-index-scratch-direct-java-migration.md), and [D4 fold/window](backends/cpu/tasks/0009d4-fold-window-route-decision-and-possible-direct-java-migration.md) are retained-generated-route decisions; D4 retains its complete 32-row family because direct replacement would increase total implementation and verification work. D1A stays completed historical hygiene evidence.
...
complete. At that point, this did not change the then-active CPU frontier: CPU generated-artifact storage,
...
whitespace checks without rerunning Java. Task 0003 and the original provider milestone remain
Complete. The later CPU planning pass selected only task 0001 as Ready. Optional provider task
0004 later reopened the provider area without altering that history; after its proof gates failed,
it remains a blocked/deferred side branch rather than the current frontier. CPU 0010E completed
the independent FLOAT32/FLOAT64 producer/consumer slice recorded above; Prepare 0004 is Complete,
and detailed tools/tuning 0001 is Complete with explicit caller-supplied identity, collaboration,
and cold execution as its resolved operational inputs.
...
which owns binding-aware target-Shape SUM. Detailed Complete
[CPU 0007B](backends/cpu/tasks/0007b-portable-arg-extrema-coverage.md) owns arg extrema, detailed
Complete [CPU 0007C](backends/cpu/tasks/0007c-portable-masked-reduction-coverage.md) owns masked
reductions. Detailed Complete
[CPU 0007D](backends/cpu/tasks/0007d-portable-logarithmic-statistical-and-norm-reduction-coverage.md)
owns advanced logarithmic/statistical/norm reductions, detailed Complete
[CPU 0007E](backends/cpu/tasks/0007e-portable-stable-softmax-and-log-softmax-coverage.md) owns
stable multi-pass softmax/log-softmax. Complete
[CPU 0007F](backends/cpu/tasks/0007f-portable-layer-and-rms-normalization-coverage.md) owns
trailing-slice Layer/RMS normalization; Complete
[CPU 0007F1](backends/cpu/tasks/0007f1-portable-batch-normalization-inference-coverage.md) then owns
batch inference; and detailed Complete
[CPU 0007F2](backends/cpu/tasks/0007f2-portable-batch-normalization-training-and-statistic-transition-coverage.md)
owns batch training/statistic transition. After Model 0025G, Model 0025H, and Compiler 0006B
completed in that order, the planning pass narrowed the former five-family Draft CPU 0008 row to
the smallest cohesive capability. Detailed Complete CPU 0008 owns grouped NCHW Conv2d, intrinsic
optional bias, direct legal external ADD and ADD-plus-RELU epilogues, and the sole tagged two-unit
materialized suffix when a bounded independently supported suffix cannot legally fuse.

The implementation resolved a narrow contradiction: the accepted split needs two
artifacts, while the existing CPU plan and finalizer assumed one unit. Complete CPU 0008 resolves
that contradiction without changing shared architecture. Only an exact bounded
illegal-but-independently-supported Conv2d suffix may select the tagged two-unit CPU-private form.
Analysis declares the connecting Conv2d result once as an ordinary buffer before shared slot
assignment; finalization realizes independently keyed Conv2d and pointwise artifacts and wraps
them in one CPU-private composite executable. Cold binding validates both units before any write,
the first unit completes before the second begins, and unchanged Runtime applies the composite's
ordinary atomic output-validity transition before the normal publication suffix. Every existing
family, direct Conv2d, and legal fused Conv2d plan remains one-unit. General DAG decomposition,
recognition, candidate enumeration, profitability, and representation planning remain in Draft
CPU 0008B–0008E in their existing order.

CPU 0008 completes the direct FLOAT64/FLOAT32/BFLOAT16 path at generator schema 51 with groups and
depthwise geometry, explicit padding/stride/dilation, resolved arrays/segments/mixed carriers,
scalar and parallel-scalar complete-output-cell ranges, and one ordinary intermediate for the
two-unit exception. Runtime sees one atomic prepared-executable access boundary; the CPU-private
sequence cold-binds and alias-validates both children, invokes them strictly in order, and does not
mutate validity itself. The authoritative uncached CPU suite passed 485 tests with three expected
skips and no failures or errors. Five accepted Java 26 fixed-heap performance forks passed every
generated/direct and aggregate-median `<= 1.15x` gate; the highest aggregate median was
`1.057987982x`. The retained evidence manifest verifies with digest
...
0008E1 and CPU 0008F are Complete.
...
Complete
[CPU 0008B General partition-DAG computation-unit decomposition and bounded fusion](backends/cpu/tasks/0008b-general-partition-dag-computation-unit-decomposition-and-bounded-fusion.md)
then replaces the straight-line/single-unit boundary and closed Conv2d exception with deterministic
one-to-eight-unit decomposition, exact ordinary-buffer DAG-edge splits, per-unit resources, and one
atomic sequential CPU-private composite. It admits bounded vertical and horizontal ordinary-
pointwise fusion only across legal edges; fan-out, publication, alias, state/random, affine/
specialized, and numerical-order barriers or any hard indexing, code-size, live-value, boundary,
unit, or attempt budget retain the complete split plan. CPU 0008B performs deterministic maximal
legal fusion, not profitability ranking. Its first-use liveness rule, hard-budget boundaries,
resource lifecycle, cold-failure behavior, generated shape, and performance matrix are complete.
Complete
[CPU 0008C Typed specialized-subgraph and epilogue recognition](backends/cpu/tasks/0008c-typed-specialized-subgraph-and-epilogue-recognition.md)
adds recognition-only CPU-private typed facts for exact MATMUL, Conv1d/Conv2d/Conv3d, selected
floating-reduction epilogues, and already first-class softmax/normalization kernels. Its closed
suffix is optional external ADD plus at most one exact activation/CLAMP. It adds no generated form
or execution: at that checkpoint MATMUL stayed unsupported pending CPU 0008F; only CPU 0008's existing Conv2d
ADD/ADD-RELU form maps to existing specialized execution; and every other recognized epilogue
retains the exact CPU 0008B split. Schema 52 and artifact identity remain unchanged. It creates no
public pattern registry or domain-specific language, adds no Model kind, and never silently
recognizes decomposed softmax, normalization, attention, or loss as a first-class semantic kind.
Detailed complete CPU 0008D ranks the admitted bounded set of legal existing fused and split
...
across the five currently executable pointwise types; BFLOAT16 waits for numerical support. A
pair is rejected when one represented instruction consumes both sources; both singles and eligible
disjoint-consumer pairs remain complete candidates. Repeated boundaries deduplicate and one copy
may serve compatible repeated uses. Existing costs remain diagnostic facts, but the retained
one-copy failure proves neither those estimates nor cross-unit reuse sufficient to promote every
admitted case. Ordinary preparation therefore selects CPU 0008D's exact direct topology and
records that materialization is unproved. CPU 0008E does not change fusion legality or invent
topologies; DAG split materialization remains CPU 0008B work, while native packing and reorder
remain owned by CPU 0010B and later route tasks. Every complete variant has a stable typed CPU-private identity
carrying its materialized boundaries, layout/access regimes, reuse, workspace, execution strategy,
and fused/split topology. Materialized candidates remain resource-complete and executable without
making their identity irreproducible for the later opaque Prepare/tuning handoff. CPU 0008E
performs no measurement, tuning-cache
lookup or mutation, public tuning configuration, or shared handoff implementation.
The schema-53 affine hot shape remains equivalent to optimal clean Java. The older
production-selectable pair row remains immutable evidence for `CO_CONSUMED_PAIR`. The newer
FLOAT64 heap one-copy fork remains immutable at SHA-256
`c55d07272272fb484b4a6580312aba34aa67483e404f42ecddaf8a717d7f4612`: generated/oracle
median `1.000326680`, selected/direct median `2.605250934`, and samples `2.615038667`,
`2.606896076`, `2.498279430`, `2.583623032`, `2.613269873`. It is not retried, replaced,
relabelled, or required to masquerade as selected. The correction retains its candidate identity
under identical facts and proves direct production selection instead. Any future static policy
claiming a materialized admitted set requires fresh no-retry five-fork complete-plan `<= 1.15x`
evidence for every represented admitted case.

Ordinary CPU preparation uses bounded deterministic no-measurement topology heuristics and direct
representation selection while materialization promotion remains unproved.
...
CPU legality, recognition, or selection. Config's immutable public tuning-request facade follows
stable consumers; Config 0004–0006A do not gate this initial exact/default slice. Complete
[CPU 0008E1](backends/cpu/tasks/0008e1-shared-partition-dag-adoption-and-reconstruction-removal.md)
...
`KEEP_SCALAR` evidence but no completed five-fork acceptance, and CPU 0008P is Complete with its
modular partial/combine implementation fail-closed at `KEEP_WHOLE_CELL`, not floating reassociation.
CPU 0009 depends on completed [CPU 0008Q1A](backends/cpu/tasks/0008q1a-vector-scalar-power-hot-path-self-containment.md) and its documentation pass, plus [CPU 0008Q1](backends/cpu/tasks/0008q1-finite-scalar-immediate-clamp-matrix.md) and its documentation pass, in addition to CPU 0008Q's narrow two-fixture constants-only mechanism proof, as well as the ordered CPU 0008A–0008E, Prepare 0003A, CPU 0008E1, and CPU 0008F–0008P
sequence
...
deduplicated reuse, and candidate/resource ceilings—before CPU 0010 and CPU 0010A–0010E add or
extend portable/native peer-route choices. The required frontier order is CPU 0008 -> CPU 0008A -> CPU 0008B -> CPU
0008C -> CPU 0008D -> CPU 0008E -> Prepare 0003A -> CPU 0008E1 -> CPU 0008F before those later
...
CPU 0008Q completed a narrow two-fixture BFLOAT16 scalar-MUL represented-immediate mechanism from exact fixture hashes and automated constants-only Class-File normalization; it neither enumerates arbitrary immediate bits nor changes production identity. CPU 0008Q1A and CPU 0008Q1 are Complete with their recorded finite schema-64 evidence. CPU 0009 is Complete: its proportional structural promotion is comparator-backed, not a literal CFG proof, and representative performance remains separate. Final 0009G reconciles all 120 live forms with the readable ledger, exact generated/rejected accounting, dense/general presence, and meaningful selected strategies. CPU 0009B is Complete for 845 rows and CPU 0009C is Complete for its exact 2,252 rows; exactly 2,252 generated rows are oracle-proved, while all 17,463 remain performance-partial. Parent CPU 0009D is Complete because D1, D1A, D2, D3, and D4 are Complete; D4 retained the complete 32-row generated fold family because a direct replacement would increase total implementation and verification work. The 173 cross-category rejections remain 152 `rejected:pointwise` plus 21 other-category rows. The fresh loss fork 0 failed 19 of 792 rows; that is retained NON_PASSING performance debt for the concise post-0009 remediation row, not a semantic or structural support failure. No whole-backend performance-parity claim closes. Detailed CPU
0008J and Model 0025L are `Complete`, detailed CPU 0008K and CPU 0008L are `Complete`; detailed
[CPU 0008M](backends/cpu/tasks/0008m-vector-mse-none.md) is `Complete`, CPU 0008N ended with a
bounded scalar stop, CPU 0008N1 is Complete, CPU 0008O is `Cancelled` with retained fork-0
`KEEP_SCALAR` evidence but no completed five-fork acceptance, and CPU 0008P is Complete with its
modular partial/combine implementation fail-closed at `KEEP_WHOLE_CELL`, not floating reassociation. The ordered CPU 0008J–0008P sequence is complete; completed 0009A--G remain historical evidence. CPU 0009D is Complete because D1 through D4 retain generated routes. CPU 0009E1, E1A, E1B1, E1B2, E1B3, E1C, E2, and E3 are Complete retained generated routes; E3 independently retains MSE, dense categorical, and index categorical generated routes because direct Java would increase implementation and verification work, without a performance claim. CPU 0009 is Complete after 0009G reconciled all live forms and meaningful selected strategies without universal structural or performance promotion. Loss fork 0 remains NON_PASSING historical evidence after user closure of forks 1--4; it is not a migration gate.

Current correction: parent CPU 0009D is Complete because D1 through D4 are Complete retained-generated-route decisions. Completed D1A remains historical. [CPU 0009E1 partial integral reduction](backends/cpu/tasks/0009e1-partial-integral-reduction-direct-java-migration.md), [CPU 0009E1A masked reduction](backends/cpu/tasks/0009e1a-masked-reduction-route-decision.md), [E1B1 log-sum-exp](backends/cpu/tasks/0009e1b1-log-sum-exp-route-decision.md), [E1B2 statistical reductions](backends/cpu/tasks/0009e1b2-statistical-reduction-route-decision.md), [E1B3 norms](backends/cpu/tasks/0009e1b3-norm-reduction-route-decision.md), [E1C softmax-style](backends/cpu/tasks/0009e1c-softmax-style-reduction-route-decision.md), [E2 normalization](backends/cpu/tasks/0009e2-normalization-route-decision-and-possible-direct-java-migration.md), and [E3 loss](backends/cpu/tasks/0009e3-loss-route-decision-and-possible-direct-java-migration.md) are Complete retained generated routes. CPU 0009 and 0009G are Complete; no conditional structural reconciliation child remains.

Completed correction: [CPU 0009G1 scalar-strategy evidence correction](backends/cpu/tasks/0009g1-scalar-strategy-evidence-correction.md) is `Complete` after CPU 0009G. It added selected-`SCALAR` witnesses only for `SCALAR_SUB`, `SCALAR_MUL`, `SCALAR_DIV`, `SCALAR_MIN`, `SCALAR_MAX`, `SCALAR_POW`, and `SCALAR_CLAMP`, retained ADD as the orchestration basis, and made selected scalar evidence fail closed for every live admitted form where scalar is meaningful. The checkpoint separately seals the exact eight-owner direct witness basis. It changed no production behavior, capability, lowering, generator schema, strategy policy, or performance fact. [CPU 0010 narrow OpenBLAS BLAS-compatible native route](backends/cpu/tasks/0010-narrow-openblas-blas-compatible-native-route.md) is `Complete`: it adds only the qualified positive rank-two same-type FLOAT32/FLOAT64 bare MATMUL route, direct native or one-input affine materialization, deterministic whole-plan cost with portable retention, and native-free backend conformance through the existing staged preparation/finalization boundary without Engine. CPU 0010A and detailed [CPU 0010B](backends/cpu/tasks/0010b-bounded-openblas-matmul-representation-expansion.md), [CPU 0010C](backends/cpu/tasks/0010c-coordinated-openblas-thread-candidates-and-shared-cpu-thread-budget.md), and [CPU 0010D](backends/cpu/tasks/0010d-installed-openblas-qualification-and-target-fingerprinting.md) are `Complete`; optional OpenBLAS provider 0004 and CPU 0010D1 are blocked/deferred BFLOAT16 side-branch work, detailed [CPU 0010E](backends/cpu/tasks/0010e-float32-float64-openblas-tuning-candidates-and-compatible-decisions.md) is `Complete` for the independent FLOAT32/FLOAT64 slice, detailed Prepare 0004 is `Complete`, detailed tools/tuning 0001 is Complete, and CPU 0011 remains separately `Blocked`.
...
`KEEP_SCALAR` evidence but no completed five-fork acceptance, and CPU 0008P is Complete with its
modular partial/combine implementation fail-closed at `KEEP_WHOLE_CELL`, not floating reassociation, while CPU 0008Q is [`Complete`](backends/cpu/tasks/0008q-scalar-immediate-clamp-generated-equivalence.md) as the narrow two-fixture mechanism, CPU 0008Q1A is [`Complete`](backends/cpu/tasks/0008q1a-vector-scalar-power-hot-path-self-containment.md), CPU 0008Q1 is [`Complete`](backends/cpu/tasks/0008q1-finite-scalar-immediate-clamp-matrix.md), CPU 0009 is [`Complete`](backends/cpu/tasks/0009-portable-generated-coverage-closure-checkpoint.md), and completed 0009A--G remain historical evidence. Parent CPU 0009D is Complete because D1 through D4 retained generated routes. CPU 0009E1, E1A, E1B1, E1B2, E1B3, E1C, E2, and E3 are Complete retained generated routes; E1B3 retains L1_NORM/L2_NORM, E1C retains SOFTMAX/LOG_SOFTMAX, E2 retains static Layer/RMS normalization, and E3 separately retains MSE, dense categorical, and index categorical generated routes because direct Java would increase implementation and verification work, without a performance claim. E1B and E1C are Complete; final 0009G reconciled all live forms with meaningful selected-strategy facts but no universal strategy, structural, or performance claim. The complete semantic inventory and 6,009 catalog remain historical inspection evidence, while retained loss fork-0 evidence remains NON_PASSING and fresh profiling is non-blocking absent a correctness regression. [CPU 0010](backends/cpu/tasks/0010-narrow-openblas-blas-compatible-native-route.md) is `Complete`; CPU 0010A and detailed [CPU 0010B](backends/cpu/tasks/0010b-bounded-openblas-matmul-representation-expansion.md), [CPU 0010C](backends/cpu/tasks/0010c-coordinated-openblas-thread-candidates-and-shared-cpu-thread-budget.md), and [CPU 0010D](backends/cpu/tasks/0010d-installed-openblas-qualification-and-target-fingerprinting.md) are `Complete`; optional OpenBLAS provider 0004 and CPU 0010D1 are blocked/deferred BFLOAT16 side-branch work, detailed [CPU 0010E](backends/cpu/tasks/0010e-float32-float64-openblas-tuning-candidates-and-compatible-decisions.md) is `Complete` for the independent FLOAT32/FLOAT64 slice, detailed Prepare 0004 is `Complete`, detailed tools/tuning 0001 is Complete, CPU 0011 remains separately `Blocked`, and CPU 0012–0017 plus the refined Config, Metal, and tuning rows remain staged `Draft` work.

Current correction: parent CPU 0009D is Complete. [CPU 0009D1 aggregate](backends/cpu/tasks/0009d1-aggregate-structural-oracle-closure.md), [CPU 0009D2 scan](backends/cpu/tasks/0009d2-scan-route-decision-and-possible-direct-java-migration.md), [D3 ordering](backends/cpu/tasks/0009d3-ordering-index-scratch-direct-java-migration.md), and [D4 fold/window](backends/cpu/tasks/0009d4-fold-window-route-decision-and-possible-direct-java-migration.md) are Complete retained-generated-route decisions. D1A remains Complete historical evidence. [CPU 0009E1 partial integral reduction](backends/cpu/tasks/0009e1-partial-integral-reduction-direct-java-migration.md), [CPU 0009E1A masked reduction](backends/cpu/tasks/0009e1a-masked-reduction-route-decision.md), [E1B1 log-sum-exp](backends/cpu/tasks/0009e1b1-log-sum-exp-route-decision.md), [E1B2 statistical reductions](backends/cpu/tasks/0009e1b2-statistical-reduction-route-decision.md), [E1B3 norms](backends/cpu/tasks/0009e1b3-norm-reduction-route-decision.md), [E1C softmax-style](backends/cpu/tasks/0009e1c-softmax-style-reduction-route-decision.md), [E2 normalization](backends/cpu/tasks/0009e2-normalization-route-decision-and-possible-direct-java-migration.md), and [E3 loss](backends/cpu/tasks/0009e3-loss-route-decision-and-possible-direct-java-migration.md) are Complete retained generated routes. E3 independently retains complete MSE, dense categorical, and index categorical generated routes because direct Java would increase implementation and verification work. CPU 0009 and 0009G are Complete.

Completed correction: [CPU 0009G1 scalar-strategy evidence correction](backends/cpu/tasks/0009g1-scalar-strategy-evidence-correction.md) is `Complete` after CPU 0009G. It added selected-`SCALAR` witnesses only for `SCALAR_SUB`, `SCALAR_MUL`, `SCALAR_DIV`, `SCALAR_MIN`, `SCALAR_MAX`, `SCALAR_POW`, and `SCALAR_CLAMP`, retained ADD as the orchestration basis, and made selected scalar evidence fail closed for every live admitted form where scalar is meaningful. The checkpoint separately seals the exact eight-owner direct witness basis. It changed no production behavior, capability, lowering, generator schema, strategy policy, or performance fact. [CPU 0010 narrow OpenBLAS BLAS-compatible native route](backends/cpu/tasks/0010-narrow-openblas-blas-compatible-native-route.md) is `Complete` with the same bounded rank-two, type, provider, representation, and whole-plan-cost boundary recorded above. CPU 0010A and detailed [CPU 0010B](backends/cpu/tasks/0010b-bounded-openblas-matmul-representation-expansion.md), [CPU 0010C](backends/cpu/tasks/0010c-coordinated-openblas-thread-candidates-and-shared-cpu-thread-budget.md), and [CPU 0010D](backends/cpu/tasks/0010d-installed-openblas-qualification-and-target-fingerprinting.md) are `Complete`; optional OpenBLAS provider 0004 and CPU 0010D1 are blocked/deferred BFLOAT16 side-branch work, detailed [CPU 0010E](backends/cpu/tasks/0010e-float32-float64-openblas-tuning-candidates-and-compatible-decisions.md) is `Complete` for the independent FLOAT32/FLOAT64 slice, detailed Prepare 0004 is `Complete`, detailed tools/tuning 0001 is Complete, and CPU 0011 remains separately `Blocked`.
...
CPU 0010E, detailed Prepare 0004, detailed tools/tuning 0001, Tuning 0002, and Tuning 0003 are
Complete. Config 0006A now
...
Config 0006B, now `Complete` under a second non-overlapping exception. It preserves every 0006A
...
through CPU 0007A1O, detailed CPU 0007A2, detailed CPU 0007B, and detailed CPU 0007C are
`Complete`. Detailed CPU 0007D, detailed CPU 0007E, and detailed CPU 0007F are `Complete`; detailed
CPU 0007F1 is `Complete`; detailed CPU 0007F2 is `Complete`, and later work remains Draft without detailed specifications.
...
cost model, shared-module/build/architecture change, or later specification. Prepare 0004 is the
bounded opaque handoff after CPU 0010E's concrete typed-candidate producer and before the
tools/tuning 0001 consumer; it does not reopen or reorder completed Prepare history.
...
Runtime milestone. Prepare 0003 and user-authorized interleaved Prepare 0003A are Complete.
Backend Contract remains Complete and closed. Module
dependency directions are unchanged; the Runtime
...
| [0006B7 Final NEG logical-layout closure](modules/compiler/tasks/0006b7-final-neg-logical-layout-closure.md) | Complete | 0006B3–0006B6; Metal 0002 prerequisite review; current Model/Planning contracts | Closed only fully static still-unresolved exact unary NEG outputs and directly consumed compile-time splat inputs after existing closures and before all final artifact derivation, without changing Model inference or creating a general layout policy. |
...
0006B7 completed after 0006B6 and before 0006C under the separate user-authorized Metal
prerequisite exception. Compiler 0006C and 0007 remain explicitly deferred Draft side branches without detailed
...
own complete valid graph and ownership candidates; shared Prepare now owns the narrow opaque
candidate/decision transport while future work owns orchestration and artifact lifecycle; CPU,
Metal, and CUDA own typed route candidate
...
remain Complete. Config task 0003 and detailed Config 0006A–0006B are also Complete. Config
0004–0006 and 0007–0008 remain ordered Draft work without detailed specifications. Planning task
0001 remains
