...
| 8 | [`modules/prepare`](modules/prepare/master-plan.md) | Complete | Compiler/planning artifacts and Runtime recipe/runner contracts are stable; ADR 0010 authorizes the analysis-first staged handoff. | Shared prepare contracts and validation include one immutable partition-local DAG projection before CPU adoption and MATMUL. |
| 9 | [`backends/openblas-provider`](backends/openblas-provider/master-plan.md) | Blocked (provider 0004) | Native interop conventions needed by the provider are decided. | Required FLOAT32/FLOAT64 remains complete; the optional direct BFLOAT16-output capability is either proved and implemented or stopped fail-closed with its evidence gap recorded. |
| 10 | [`backends/cpu`](backends/cpu/master-plan.md) | In progress (CPU 0010D Complete; CPU 0010D1 Draft and non-executable; CPU 0010E Draft) | Model, config, planning, runtime, prepare, backend-contract, trace, and applicable OpenBLAS contracts are ready. | CPU is a conforming reference backend for the selected capability set. |
...
is `Blocked`: pinned OpenBLAS v0.3.31 fails both the exported-C-ABI and one-final-BFLOAT16-
narrowing gates. [CPU 0010D installed OpenBLAS qualification and target
fingerprinting](backends/cpu/tasks/0010d-installed-openblas-qualification-and-target-fingerprinting.md)
is `Complete`; dependent CPU 0010D1 remains a non-executable master-plan-only `Draft`, and CPU
0010E remains after it as another master-plan-only `Draft`, with no detailed task file for either.
The global ordered frontier is blocked with no next in-scope task: advancing CPU 0010D1, CPU
0010E, CPU 0011, or a later project area would silently skip the unresolved provider dependency
or recorded order.
...
schema, or architecture rule is authorized. Future CPU 0010E, Prepare 0004, narrowed CPU 0016,
and Tuning 0001–0002 own compatible end-to-end promotion. The first full corrective CPU rerun is
...
follows 0008E1 and now owns bounded portable MATMUL execution. Existing Prepare 0004
...
0004 later reopened the provider area without altering that history and is now the blocked
detailed frontier after its proof gates failed.
...
The distinct future Config 0006A request inputs, CPU 0010E OpenBLAS candidates and evidence,
Prepare 0004 opaque candidate handoff, narrowed CPU 0016 cross-route cache integration, and
Tuning 0001–0002 measurement/selection workflow may later measure eligible complete materialized
...
Completed correction: [CPU 0009G1 scalar-strategy evidence correction](backends/cpu/tasks/0009g1-scalar-strategy-evidence-correction.md) is `Complete` after CPU 0009G. It added selected-`SCALAR` witnesses only for `SCALAR_SUB`, `SCALAR_MUL`, `SCALAR_DIV`, `SCALAR_MIN`, `SCALAR_MAX`, `SCALAR_POW`, and `SCALAR_CLAMP`, retained ADD as the orchestration basis, and made selected scalar evidence fail closed for every live admitted form where scalar is meaningful. The checkpoint separately seals the exact eight-owner direct witness basis. It changed no production behavior, capability, lowering, generator schema, strategy policy, or performance fact. [CPU 0010 narrow OpenBLAS BLAS-compatible native route](backends/cpu/tasks/0010-narrow-openblas-blas-compatible-native-route.md) is `Complete` with the same bounded rank-two, type, provider, representation, and whole-plan-cost boundary recorded above. CPU 0010A and detailed [CPU 0010B](backends/cpu/tasks/0010b-bounded-openblas-matmul-representation-expansion.md), [CPU 0010C](backends/cpu/tasks/0010c-coordinated-openblas-thread-candidates-and-shared-cpu-thread-budget.md), and [CPU 0010D](backends/cpu/tasks/0010d-installed-openblas-qualification-and-target-fingerprinting.md) are `Complete`; OpenBLAS provider 0004 is the blocked detailed frontier, CPU 0010D1 is the dependent non-executable master-plan-only `Draft`, CPU 0010E follows it as a master-plan-only `Draft`, and CPU 0011 remains `Blocked` after that sequence.

Prepare 0003A is
Complete.
Completed OpenBLAS tasks and original milestone history remain unchanged. Optional OpenBLAS
provider 0004 is the blocked detailed frontier. CPU 0010D1 is the dependent, non-executable
master-plan-only `Draft`, and CPU 0010E follows CPU 0010D1 as a master-plan-only `Draft`; no later
task advances around them.

OpenBLAS provider remains the blocked global project area at task 0004; CPU remains the dependent
next area and has no executable frontier. CPU 0005A through CPU 0006 are `Complete`, detailed
