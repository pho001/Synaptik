...
| 8 | [`modules/prepare`](modules/prepare/master-plan.md) | Complete baseline; Prepare 0004 is the next Draft planning frontier | Compiler/planning artifacts and Runtime recipe/runner contracts are stable; ADR 0010 authorizes the analysis-first staged handoff. | Shared prepare contracts and validation include one immutable partition-local DAG projection before CPU adoption and MATMUL. |
...
| 22 | [`tools/tuning`](tools/tuning/master-plan.md) | Draft | CPU 0010E and Prepare 0004 stabilize the first opaque candidate path; an operational cold measurement path and model identity are available. | One model-autotuning workflow reuses compatible local workload results, selects a bounded complete plan, and writes explicit validated caches or prepared-plan records before runtime. |
...
Prepare 0004 is now the single global Draft planning frontier. It has no detailed task
specification and is not yet Ready. The acyclic mainline remains CPU 0010E -> Prepare 0004 ->
tools/tuning 0001: CPU defines the typed candidates and canonical compatibility facts it owns;
Prepare transports candidates and decisions opaquely without measurement; tuning finally owns
measurement, selection, persistence, corruption/incompatibility rejection, and rich evidence.
Tools/tuning 0001 and Config 0006A remain staged Draft work. Runtime never selects.
...
follows 0008E1 and now owns bounded portable MATMUL execution. Existing Prepare 0004 retains its
completed 0003A dependency, remains Draft without renumbering or a detailed specification, and is
now staged after CPU 0010E.
...
the independent FLOAT32/FLOAT64 producer/consumer slice recorded above; Prepare 0004 is next for
detailed planning.
...
not alter completed CPU 0010E or block its FLOAT32/FLOAT64 mainline through Draft Prepare 0004 and
staged Draft tools/tuning 0001.

CPU 0010E is Complete, and Prepare 0004 is the next Draft planning frontier without a detailed
task specification. CPU 0005A through CPU 0006 are `Complete`, detailed
