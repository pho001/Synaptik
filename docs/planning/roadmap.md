...
scatter, and 20 random); parent [CPU 0009D](backends/cpu/tasks/0009d-aggregate-scan-ordering-fold-structural-oracle-closure.md) remains Ready because [CPU 0009D1 aggregate](backends/cpu/tasks/0009d1-aggregate-structural-oracle-closure.md), detailed [CPU 0009D2 scan](backends/cpu/tasks/0009d2-scan-route-decision-and-possible-direct-java-migration.md), and [D3 ordering](backends/cpu/tasks/0009d3-ordering-index-scratch-direct-java-migration.md) are Complete retained-generated-route decisions while detailed [D4 fold/window](backends/cpu/tasks/0009d4-fold-window-route-decision-and-possible-direct-java-migration.md) is the sole Ready frontier. D1 preserves the complete ordinary aggregate generated route because direct migration would duplicate exact primitive-limb/scratch machinery or add partial-split routing and evidence complexity. D2 preserves the complete generated scan route because direct Java would duplicate typed carrier/layout bodies, BFLOAT16 per-value conversion, complete-slice range ownership, and the guarded segment form while adding replacement and retirement evidence. D3 preserves the 192-row generated ordering route because direct Java would duplicate typed carrier/layout forms, stable comparison, logical indices, unsorted `TOP_K`, cold binding, two-region scratch, and replacement/retirement proof. D4 decides the complete 32-row fold family all-or-nothing, allowing finite direct Java only where it reduces total implementation and verification work. None makes a performance claim. 0009E splits static reductions, normalization, and loss; 0009F makes family-specific hybrid decisions; 0009G is the final support/correctness/hygiene/inventory checkpoint. Existing performance facts remain reported, but no new benchmark gate blocks correctness closure.
...
CPU 0008Q completed a narrow two-fixture BFLOAT16 scalar-MUL represented-immediate mechanism from exact fixture hashes and automated constants-only Class-File normalization; it neither enumerates arbitrary immediate bits nor changes production identity. CPU 0008Q1A and CPU 0008Q1 are Complete with their recorded finite schema-64 evidence. CPU 0009 remains Ready and incomplete: its proportional structural promotion is comparator-backed, not a literal CFG proof, and representative performance remains separate. CPU 0009B is Complete for 845 rows and CPU 0009C is Complete for its exact 2,252 rows; exactly 2,252 generated rows are oracle-proved, while all 17,463 remain performance-partial. Parent CPU 0009D remains Ready because D1, D1A, D2, and D3 are Complete while detailed D4 fold/window is the sole Ready frontier. The 173 cross-category rejections remain 152 `rejected:pointwise` plus 21 other-category rows. The fresh loss fork 0 failed 19 of 792 rows; that is retained NON_PASSING performance debt for the concise post-0009 remediation row, not a semantic or structural support failure. No whole-backend performance-parity claim closes. Detailed CPU
0008J and Model 0025L are `Complete`, detailed CPU 0008K and CPU 0008L are `Complete`; detailed
[CPU 0008M](backends/cpu/tasks/0008m-vector-mse-none.md) is `Complete`, CPU 0008N ended with a
bounded scalar stop, CPU 0008N1 is Complete, CPU 0008O is `Cancelled` with retained fork-0
`KEEP_SCALAR` evidence but no completed five-fork acceptance, and CPU 0008P is Complete with its
modular partial/combine implementation fail-closed at `KEEP_WHOLE_CELL`, not floating reassociation. The ordered CPU 0008J–0008P sequence is complete; completed 0009A--C/D1/D1A/D2/D3 remain historical evidence. CPU 0009D remains Ready because D1, D2, and D3 retain their generated routes while detailed D4 fold/window is the sole Ready frontier. CPU 0009E is split into reductions/normalization/loss, 0009F uses explicit hybrid family decisions, and 0009G is the final support/correctness/hygiene/inventory checkpoint. Loss fork 0 remains NON_PASSING historical evidence after user closure of forks 1--4; it is not a migration gate.

Current correction: parent CPU 0009D remains Ready because D1, D2, and D3 are Complete retained-generated-route decisions while detailed D4 fold/window is the sole Ready frontier; completed D1A remains historical. No conditional structural reconciliation child remains.
