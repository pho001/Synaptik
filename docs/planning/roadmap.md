...
| 9 | [`backends/openblas-provider`](backends/openblas-provider/master-plan.md) | In progress (provider 0004 Ready) | Native interop conventions needed by the provider are decided. | Required FLOAT32/FLOAT64 remains complete; the optional direct BFLOAT16-output capability is either proved and implemented or stopped fail-closed with its evidence gap recorded. |
| 10 | [`backends/cpu`](backends/cpu/master-plan.md) | In progress (CPU 0010D Complete; CPU 0010D1 Draft; CPU 0010E Draft) | Model, config, planning, runtime, prepare, backend-contract, trace, and applicable OpenBLAS contracts are ready. | CPU is a conforming reference backend for the selected capability set. |
...
is the sole detailed `Ready` frontier. [CPU 0010D installed OpenBLAS qualification and target
fingerprinting](backends/cpu/tasks/0010d-installed-openblas-qualification-and-target-fingerprinting.md)
is `Complete`; dependent CPU 0010D1 and then CPU 0010E remain master-plan-only `Draft` rows with
no detailed task files.
...
0004 now reopens the provider area as the sole detailed `Ready` frontier without altering that
history.
...
provider 0004 is the sole detailed `Ready` frontier. CPU 0010D1 is the dependent master-plan-only
`Draft`, and CPU 0010E follows CPU 0010D1 as a master-plan-only `Draft`.

OpenBLAS provider is the active global project area for task 0004; CPU remains the dependent next
area. CPU 0005A through CPU 0006 are `Complete`, detailed
