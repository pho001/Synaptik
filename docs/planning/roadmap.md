...
| 10 | [`backends/cpu`](backends/cpu/master-plan.md) | In progress (CPU 0010D Complete; CPU 0010E Draft) | Model, config, planning, runtime, prepare, backend-contract, trace, and OpenBLAS contracts are ready. | CPU is a conforming reference backend for the selected capability set. |
...
is `Complete`. CPU 0010E is the active master-plan-only `Draft` frontier; no detailed 0010E task
exists.
...
Complete. The OpenBLAS provider project area remains Complete.
