...
| 0009E1 | [Partial integral reduction direct-Java migration](tasks/0009e1-partial-integral-reduction-direct-java-migration.md) | Ready | 0009D4 | Sole detailed frontier: decide/migrate the complete current dense-array INT32/INT64 modular SUM/PROD partial route only when direct Java lowers total code/evidence complexity; no benchmark gate. |
| 0009E1A | Masked reduction route decision | Draft | 0009E1 | Summary only: current masked FLOAT64/FLOAT32/BFLOAT16 SUM/MEAN, directional BOOL broadcast, exact-state workspace, and typed carrier/layout matrix. |
...
completed documentation pass. CPU 0009 remains Ready and incomplete. Completed 0009A--C/D1/D1A/D2/D3/D4 remain historical evidence. Parent CPU 0009D is Complete: D1 retained aggregate generation because direct migration would duplicate exact primitive-limb/scratch machinery or add partial-split complexity; D2 retained scan generation because a direct replacement would duplicate typed carrier/layout, BFLOAT16, complete-slice range, and guarded segment bodies while adding retirement evidence; D3 retained ordering because a direct route would duplicate the 192-row typed carrier/layout, stable-comparison, logical-index, scratch, cold-binding, and retirement-proof topology; and D4 retained the complete 32-row fold family because a direct route would duplicate five typed axis and three typed 2D bodies over four carrier combinations, dense/general forms, cold selection/binding, output ranges, exact mapping/rejections, BFLOAT16 per-add rounding, integral wrap, and replacement/invocation/retirement evidence. CPU 0009E1 partial integral reduction is the sole detailed Ready frontier; E1A masked, E1B advanced, E1C softmax-style, E2 normalization, and E3 loss remain ordered summaries, followed by 0009F hybrid decisions and the 0009G support/correctness/hygiene/inventory checkpoint. Exact rejections remain. Existing performance evidence, including the loss fork-0 NON_PASSING result and user-closed forks 1--4, is retained but fresh benchmarking is non-blocking. CPU 0010 through 0017
