...
| 0008N1 | Generated Conv width-block loop/dataflow parity re-spike | Draft | 0008N bounded stop | Re-spike a generated nested width-block loop that matches the optimal direct Java dataflow instead of the observed per-block complete-coordinate decode and repeated long geometry/address work, then restart all fixed 0008N gates without weakening `0.90x` or `1.15x`. The observed dataflow mismatch plausibly explains the failed parity row, but its individual costs are not yet isolated. No detailed task specification exists yet. |
...
generated/direct parity at `1.775769426x`. CPU 0008N1 is the next Draft remediation frontier;
...
[CPU 0008M](tasks/0008m-vector-mse-none.md) is `Complete`; detailed
[CPU 0008N](tasks/0008n-measured-profitable-float32-float64-conv2d-conv3d-simd-accumulation.md)
is Incomplete after its bounded scalar stop; CPU 0008N1 is the next Draft implementation frontier.
...
stop, CPU 0008N1 is its Draft remediation, CPU 0008O and CPU 0008P remain ordered Draft follow-ups,
...
[CPU 0008L](tasks/0008l-pointwise-simd-mask-output-closure.md) are `Complete`; detailed
[CPU 0008M](tasks/0008m-vector-mse-none.md) is `Complete`; CPU 0008N ended with a bounded scalar
stop and CPU 0008N1 is the next Draft remediation frontier.
These notes do
...
- Complete CPU 0008 is narrowed from the former five-family Draft row to one grouped NCHW
  ...
  Draft remediation frontier and all Conv execution remains scalar at schema 62. This planning
