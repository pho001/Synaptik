...
| 0007A1K | [Affine-copy residual parity](tasks/0007a1k-affine-copy-residual-parity.md) | Ready | 0007A1J | Correct persistent raw-BFLOAT16 `A-GENERAL` through sole owner `CpuAffineCopyEmitter`, preserving composed PERMUTE/SLICE mapping, segment-to-heap carriers, offsets/strides, arbitrary legal output ranges, zero workspace, and typed fallback. |
| 0007A2 | Portable binding-aware sum-to-Shape reduction | Draft | 0007A1C; 0007A1D–0007A1K | Add `SUM` with exact `SumToShapeAttrs`, right-aligned bound-Shape validation, leading/aligned reduction geometry, all five numeric types, and truthful resources without dynamic unresolved execution. |
...
0007A1F through CPU 0007A1J are `Complete`; detailed CPU 0007A1K is the sole next ordered
...
through the explicit Conv2d composition; it is not an authorization to bypass CPU 0007A1K or the
...
exactly `P-SCALAR-GENERAL`, `A-GENERAL`, `S-GENERAL-MIN`, and `X-MIN-MULTI`. Detailed Ready CPU
0007A1K now owns `A-GENERAL` through `CpuAffineCopyEmitter`; the other three residual targets
remain Draft and unassigned, and CPU 0007A2 remains blocked.
...
0007A1F through CPU 0007A1J are `Complete`, and detailed CPU 0007A1K is the sole next `Ready`
...
0007A1F through CPU 0007A1J are `Complete`, detailed CPU 0007A1K is the sole next `Ready`
frontier,
...
- The former broad 0006B row is dependency-split. Detailed Complete 0006B owns only value-blind
  ...
  0007A1D, and corrective 0007A1E–0007A1K precede Draft 0007A2, which owns binding-aware
  ...
  normalization. CPU 0007A1K is the latest detailed specification and sole `Ready` task.
...
- CPU 0007A0A–0007A0F extend corrective parity coverage across every generated family that existed
  ...
  0007A1I owns the indexing group, Complete CPU 0007A1J owns the scan row, and Ready CPU 0007A1K
  owns the affine-copy row, while other residual owners remain Draft until supported by new
  evidence. CPU 0007A1K alone has a detailed `Ready`
