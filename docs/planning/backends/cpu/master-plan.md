...
| 0009D2 | [Scan route decision and possible direct-Java migration](tasks/0009d2-scan-route-decision-and-possible-direct-java-migration.md) | Ready | 0009D1 | Current detailed frontier: migrate only if the full scan matrix is genuinely cheaper to implement and verify than retained generation; otherwise retain generation explicitly. |
...
completed documentation pass. CPU 0009 remains Ready and incomplete. Completed 0009A--C/D1/D1A remain historical evidence. Parent CPU 0009D remains Ready: D1 retained the complete ordinary aggregate generated route because direct migration would duplicate exact primitive-limb/scratch machinery or add partial-split complexity. D2 scan is the detailed Ready route-decision frontier; D3 ordering and D4 fold remain summary-only. CPU 0009E is split into summary reductions, normalization, and loss children; CPU 0009F makes explicit hybrid decisions for retained generated MATMUL/convolution and pooling/attention/batch-norm orchestration; CPU 0009G is final support/correctness/hygiene/inventory closure. Exact rejections remain. Existing performance evidence, including the loss fork-0 NON_PASSING result and user-closed forks 1--4, is retained but fresh benchmarking is non-blocking. CPU 0010 through 0017
