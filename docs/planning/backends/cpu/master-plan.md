...
| 0007A1L | [Pointwise general-loop residual parity](tasks/0007a1l-pointwise-general-loop-residual-parity.md) | Ready | 0007A1K | Correct persistent `P-SCALAR-GENERAL` through the cohesive pointwise body/loop-shaping boundary while preserving exact FLOAT32 DIV/stable-SIGMOID/MUL semantics, mixed carriers, broadcast input, strided output, arbitrary legal ranges, and typed fallback. |
| 0007A2 | Portable binding-aware sum-to-Shape reduction | Draft | 0007A1C; 0007A1D–0007A1L | Add `SUM` with exact `SumToShapeAttrs`, right-aligned bound-Shape validation, leading/aligned reduction geometry, all five numeric types, and truthful resources without dynamic unresolved execution. |
...
0007A1F through CPU 0007A1K are `Complete`; detailed CPU 0007A1L is the sole next ordered
...
through the explicit Conv2d composition; it is not an authorization to bypass CPU 0007A1L or the
...
0007A1C is therefore `Review needed`. CPU 0007A1D retains its schema-32 invocation-local segment-
...
accepted fork and median. Exactly `P-SCALAR-GENERAL`, `S-GENERAL-MIN`, and `X-MIN-MULTI`
remain. Detailed Ready CPU 0007A1L owns `P-SCALAR-GENERAL`; the other two residual targets remain
...
0007A1F through CPU 0007A1K are `Complete`, and detailed CPU 0007A1L is the sole next `Ready`
...
0007A1F through CPU 0007A1K are `Complete`, detailed CPU 0007A1L is the sole next `Ready`
frontier,
...
- The former broad 0006B row is dependency-split. Detailed Complete 0006B owns only value-blind
  ...
  0007A1D, and corrective 0007A1E–0007A1L precede Draft 0007A2, which owns binding-aware
  ...
  normalization. CPU 0007A1L is the latest detailed specification and sole `Ready` task.
...
- CPU 0007A0A–0007A0F extend corrective parity coverage across every generated family that existed
  ...
  owns the affine-copy row, and Ready CPU 0007A1L owns the pointwise general row, while the two
  other residual owners remain Draft until supported by new evidence. CPU 0007A1L alone has a detailed `Ready`
