...
| `Col2Im` | Rank-3 column input mapped to `Tensor.fold2d`. Import/export supports the standard 2-D form with constant `image_shape` and `block_shape` inputs plus static `strides`, symmetric `pads`, and `dilations`. ONNX has no standard reverse `Im2Col`/`Unfold` op, so Synaptik `UNFOLD2D` and `UNFOLD_AXIS` are not exported as ONNX custom ops. |
...
- ONNX `Col2Im` is the only standard ONNX window-materialization operator that maps directly to the new Synaptik window primitives. It maps to `FOLD2D`. The reverse direction, `UNFOLD2D`/`Im2Col`, and the general `UNFOLD_AXIS` primitive have no standard `ai.onnx` operator and are deliberately not serialized as custom-domain nodes.
