...
This distinction matters. For example, `Pad`, `Tile`, `ReduceProd`, `ArgMax`, and the scoped non-negative `GatherND` subset now have Metal rows, while `ScatterND` is still a valid ONNX import/export row that executes on CPU and remains an explicit GPU coverage blocker. `Split`, `Shape`, `Size`, `ConstantOfShape`, and `Range` are static or import-boundary rows; they should not be read as native GPU operation promises. Conversely, Metal supports internal operations such as SDPA, selected losses, and backward-adjacent ops that are not ONNX interchange rows yet.
