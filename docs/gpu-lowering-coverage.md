...
| window layout materialization | `UNFOLD_AXIS`, `UNFOLD2D`, `FOLD2D` | supported | `SUPPORTED`; scoped dense `FLOAT32/BFLOAT16` native MPSGraph lowering supports `UNFOLD_AXIS` with `step=1` and `UNFOLD2D`/`FOLD2D` with `stride=1`, `dilation=1`. Wider geometry remains a visible planner rejection. |
| backward layout writes | `SLICE_BACKWARD` | supported | `SUPPORTED`; scoped static dense `FLOAT32`/`BFLOAT16` `step=1` slice backward writes lower to zero-fill pad with explicit before/after attributes. Strided slice backward writes and unsupported dtypes remain visible rejections. |
...
| conv/pool | `CONV2D` | supported | `SUPPORTED`; direct dense `FLOAT32` NCHW/OIHW forward Conv2D lowers to MPSGraph `convolution2D` for groups=1, dilation=1, stride/padding, and optional bias |
...
| conv/pool backward DAG | `UNFOLD2D`, `FOLD2D`, `ARGMAX`, `SCATTER_ELEMENTS`, `MATMUL` | backend-specific | Conv and pool backward are represented by canonical Tensor DAG primitives instead of public direct conv/pool backward op rows; Metal now lowers the scoped dense stride=1/dilation=1 window materialization and fold accumulation subset natively. |
...
| window layout materialization | `UNFOLD_AXIS`, `UNFOLD2D`, `FOLD2D` | supported | `SUPPORTED`; scoped dense `FLOAT32` native CUDA buffer kernels support `UNFOLD_AXIS` and `UNFOLD2D`/`FOLD2D` with dilation=1. Unsupported dtypes, ranks, and geometry remain visible planner rejections. |
| softmax/log-softmax-ish flows | `SOFTMAX` | supported | `SUPPORTED` |
| softmax/log-softmax-ish flows | `LOG_SOFTMAX` | supported | `SUPPORTED`; lowered as SOFTMAX followed by LOG |
...
| conv/pool | `CONV2D`, `MAX_POOL2D`, `AVG_POOL2D` | unsupported | `CAPABILITY_MISSING`; Phase 42 validates dense `FLOAT32` NCHW/OIHW forward conv/pool contracts, groups, dilation, shape/rank/layout, and average-pool divisor blockers before rejecting |
| conv/pool backward DAG | `UNFOLD2D`, `FOLD2D`, `ARGMAX`, `SCATTER_ELEMENTS`, `MATMUL` | backend-specific | CUDA support follows the primitive rows used by the canonical backward DAG rather than direct conv/pool backward rows. |
...
Forward reductions (`SUM`, `MEAN`, `REDUCE_MIN`, `REDUCE_MAX`, Metal `REDUCE_PROD`, and Metal `ARGMAX`) plus Metal `CUMSUM` scan and legal dense `FLOAT32/BFLOAT16` normalization (`LAYER_NORM`, `RMS_NORM`) are now supported for the `target=layer_norm_small` and BF16 coverage paths. Metal `ARGMAX` is scoped to dense floating inputs and public `INT64` index outputs. Unsupported normalization variants still reject explicitly: unsupported dtype uses `UNSUPPORTED_DTYPE`, non-dense direct inputs use `UNSUPPORTED_LAYOUT`, and invalid rank or tail parameter shape uses `UNSUPPORTED_RANK_OR_SHAPE`. Scoped Metal `CONV2D` is supported for dense `FLOAT32/BFLOAT16` rank-4 NCHW/OIHW tensors; unsupported Conv2D variants still reject for dtype mismatch, layout, rank/shape, groups, dilation, or metadata encoding limits. Conv backward is represented as canonical Tensor DAG nodes using window layout and matrix multiplication primitives; scoped `UNFOLD2D` and `FOLD2D` now lower natively for dense stride=1/dilation=1 geometry, while wider geometry remains a visible planner rejection. Scoped Metal direct `MAX_POOL2D` and `AVG_POOL2D` are supported for dense `FLOAT32/BFLOAT16` NCHW pooling with `AVG_POOL2D` scoped to `countIncludePad=false`; pool backward is represented as canonical Tensor DAG nodes using the same scoped window lowering rows. Dense Metal loss rows are supported for `NLL_LOSS` and dense `CROSS_ENTROPY_LOSS`; index-target `CROSS_ENTROPY_LOSS_INDICES` and `CROSS_ENTROPY_LOSS_INDICES_GRAD` are supported for dense `FLOAT32/BFLOAT16` logits/sampleScale, dense static in-bounds `INT32` targets, public ignore-index masking, and `NONE`/`SUM`/`MEAN` reductions.
...
Phase 27 expands the matrix surface for conv/pool and BOOL-producing operations without claiming native GPU execution prematurely. Metal and CUDA list the direct forward conv/pool ops explicitly: `CONV2D`, `MAX_POOL2D`, and `AVG_POOL2D`. Phase 35 promotes scoped Metal `CONV2D`, `MAX_POOL2D`, and `AVG_POOL2D` rows to supported after DAG lowering, MPSGraph buffer execution, and prepared execution evidence. Conv and pool backward no longer have direct op rows; they are expressed through canonical Tensor DAG primitives.
...
Training support is per backward operation or canonical backward DAG primitive, not inherited from forward support.
`GpuTargetCoverageTruth` now carries explicit backward rows for `SOFTMAX_GRAD`, `LOG_SOFTMAX_GRAD`,
`REDUCE_MIN_GRAD`, `REDUCE_MAX_GRAD`, `MIN_GRAD`, `MAX_GRAD`, `SCALED_DOT_PRODUCT_ATTENTION_BACKWARD`, index
gradients, and index-target loss gradients. Conv and pool backward support is represented through the canonical DAG
primitive rows such as `UNFOLD2D`, `FOLD2D`, `ARGMAX`, `SCATTER_ELEMENTS`, and `MATMUL`, not direct conv/pool
backward operation rows. Metal marks only rows with existing prepared-execution, native buffer trace, and parity
evidence as `NATIVE_EXECUTABLE`. `SCALED_DOT_PRODUCT_ATTENTION_BACKWARD` is scoped to FLOAT32 rank-3/4 SDPA producers
and supports unmasked, dense external BOOL masked, causal, and external+causal mask modes. CUDA rows remain
