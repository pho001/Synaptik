...
Forward `SLICE` and backward `SLICE_GRAD` are separate contracts. `SLICE` reads a static dense `step=1` window from a tensor. `SLICE_GRAD` takes the gradient of that window and returns a dense tensor shaped like the original input: values inside the slice window come from the incoming gradient and values outside the window are exact zeros. The supported Metal subset lowers `SLICE_GRAD` to MPSGraph constant pad with compile-time before/after attributes. `step != 1`, invalid axes/bounds, non-dense output writes, and unsupported dtypes reject before native execution.
