...
| Index write/gradient duplicate accumulation unproven | `GpuLoweringCoverageMatrix`, `MetalPartitionSupport` | `GATHER_GRAD`, `TAKE_ALONG_AXIS_GRAD`, and `SCATTER_ADD` reject with `UNSUPPORTED_DUPLICATE_INDEX` until native execution proves CPU-compatible accumulation and bounds semantics. |
