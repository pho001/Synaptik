...
Current dtype boundary: the Metal MPS FFM bridge keeps the legacy `_f32` path and adds a dtype ABI v3 compile path for widened metadata. The Java planner and bridge currently accept `FLOAT32/BFLOAT16` compute/output tensors for Metal-supported floating operation families, dtype-matched `FLOAT32/BFLOAT16` data inputs, value-preserving unary math (`ERF`, `FLOOR`, `CEIL`, `SIGN`), `BOOL` predicate inputs such as the `where` condition and direct SDPA input 3, scoped BOOL-producing compare/logical/reduction outputs, `INT32` external index inputs for supported gather/take/scatter/loss index roles, scoped `INT64` `ARGMAX` outputs, explicit `CAST` only for identity and `FLOAT32 <-> BFLOAT16` pairs, and scoped `SLICE_BACKWARD` for static dense `step=1` backward layout writes. Public `ARGMAX` now returns `INT64`, so the older scoped `INT32` index-output path is not a current graph support claim. Direct FLOAT32/BFLOAT16 rank-3/4 SDPA is supported after native MPSGraph primitive-DAG scale and mask parity verification for unmasked, dense external BOOL mask, causal, and external+causal effective mask modes. Dense FLOAT32/BFLOAT16 conv/pool forward and backward rows are supported through MPSGraph under their scoped rank/layout/parameter gates. Non-dense or unsupported SDPA mask layouts remain explicit rejection cases. `FLOAT64`, generic `INT32` compute/output, generic `INT64` compute/output outside `ARGMAX`, unsupported BOOL consumers, unsupported cast pairs such as `FLOAT32 -> INT32`, strided `SLICE_BACKWARD`, grouped/dilated conv, dtype-mismatched floating inputs, and `AVG_POOL2D countIncludePad=true` should remain on CPU or reject with visible fallback/rejection evidence.
...
2. The operation family must be one of the Metal-supported floating families documented in [Metal Backend: Supported Operations And DTypes](metal-backend.md#supported-operations-and-dtypes).
...
6. CUDA fallback interpretation starts with the generic trace summary fields `fallbackOccurred`, `fallbackKind`, `fallbackReasonCode`, and `fallbackReason`. If you need backend-local detail, continue with `acceleratorBufferReasonCode`, `cudaFallbackReason`, and CUDA trace and benchmark report fields such as `cudaExecutionPath`, `acceleratorInputBytes`, and `acceleratorNativeDeviceCopyNs`. For graph-output reads, check `RunTrace.cpuMaterializations()` and `cpuMaterializationCount`.
