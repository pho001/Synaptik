...
/**
 * Carries the normalized data-axis position shared by axis-gather and Gather-compatible
 ...
 * or {@link AxisGatherKind#GATHER_ELEMENTS}, or with {@link AxisScatterKind#SCATTER_ADD}.
 * Gather operations have ordered logical inputs {@code [data, indices]}; scatter-add has ordered
 * logical inputs {@code [data, indices, updates]}. Each kind determines its own index alignment
 * and result-shape relationship. Generic operation composition retains
 ...
 * unchanged. The current public axis-gather and Gather-compatible scatter-add Tensor-expression
 * contracts own caller-facing negative-axis normalization, rank and shape checks, and the
 * requirement that index tensors use {@code INT32} or {@code INT64}. This attributes value reads
 * no index values and therefore performs no index-value bounds check.</p>
 *
...
public record IndexAxisAttrs(int axis) implements OperationAttrs {
    /**
     * Creates immutable normalized axis parameters for an axis-gather or Gather-compatible
