...
/**
 * Constructs locally validated, storage-free NCDHW fixed-count average-pooling expressions.
 *
 ...
 * binding obligations in exact dimension expressions and never materializes the three-factor
 * divisor. It records average semantics but reads no values, creates no gradient rules, captures
 * no graph, chooses no algorithm or backend, allocates no storage, and does not execute.</p>
 */
final class TensorAveragePool3dExpressions {
    /** Prevents instantiation because average-pooling construction is stateless. */
    private TensorAveragePool3dExpressions() {
    }

    /**
     * Creates one fixed-count average-pooling expression for a floating rank-five NCDHW input.
     *
     * <p>For each spatial input extent {@code X}, kernel-position count {@code k}, padding per side
     ...
     */
    static Tensor apply(Tensor input, AveragePool3dAttrs attrs) {
        ...
        if (!dataType.isFloating()) {
            throw new IllegalArgumentException(
                    "averagePool3d input must have a floating data type, but was " + dataType);
        }

        Shape inputShape = input.descriptor().shape();
        if (inputShape.rank() != 5) {
            throw new IllegalArgumentException(
                    "averagePool3d input rank must be 5: " + inputShape.rank());
        }
        ...
        Operation operation = new Operation(Pool3dKind.AVERAGE_POOL3D, attrs);
    ...
    private static Dimension outputDimension(
        ...
        if (input instanceof StaticDimension staticInput) {
            ...
            if (numerator < 0) {
                throw new IllegalArgumentException(
                        "averagePool3d effective kernel does not fit padded " + axis + ": input="
