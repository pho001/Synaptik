...
/**
 * Constructs locally validated storage-free numeric and boolean aggregate tensor expressions.
 *
 ...
 * provenance for the supported ordinary aggregate kinds. Full forms produce the canonical scalar
...
final class TensorReductionExpressions {
     ...
     *
     * @param input non-null eligible tensor retained by exact reference as the sole provenance
     ...
     *     data type is ineligible for that kind, checked in that order with the documented message
     ...
     * validates the input for that kind before axis handling. The input Shape then normalizes the
     * caller axis exactly once. Result-shape derivation either removes that normalized position or
     ...
     *     with the parameter name as the message
     * @throws IllegalArgumentException if {@code kind} is unsupported by this helper or the input
     *     data type is ineligible for that kind, checked in that order before axis validation
    ...
    /**
     * Restricts this helper to its seven ordinary aggregate kinds.
     *
     ...
     *     {@code PROD}, {@code MIN}, {@code MAX}, {@code ALL}, or {@code ANY}, with the exact
     *     rejected enum value in the message
     */
    ...
    /**
     * Enforces the kind-specific input boundary without conversion or promotion.
     *
