/**
 * Defines prepared-execution admission plus cold creation, ownership, structural residency,
 * validity, binding, and cleanup for one complete logical run, binds exact prepared publication
 * coordinates, and owns the completed result lease.
 *
 ...
 * and leases the whole state cleanup lifecycle. It exposes only borrowed result-indexed
 * representation references while open, not storage access or host output values. Empty results
 * are valid; partial publication transfers no cleanup responsibility.
 *
 * <p>{@link io.github.pho001.synaptik.runtime.run.PreparedExecutionRunner} performs the complete
 * synchronous lifecycle. Before inspecting caller inputs or creating state, it acquires an opaque
 * lease on the prepared execution so close can reject new work without interrupting admitted
 * work. It then creates one isolated state, cold-binds every remaining occurrence before the
 * first action, traverses direct bound actions in schedule order, applies declared executable
 * read/write validity, and either leases the state to a result or closes it after failure. Lease
 * cleanup preserves an existing run failure; a persistent-resource failure after successful
 * execution closes the new result before propagating. The stateless runner may serve concurrent
 * calls whose mutable resources remain isolated. This package does not inspect storage, choose a
 * backend, infer physical aliasing or coherence, pool resources, emit trace events, or expose
