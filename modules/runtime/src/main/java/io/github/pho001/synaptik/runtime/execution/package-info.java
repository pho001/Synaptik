 ...
 * state and direct concrete typed resource references for its prepared region. Each buffer
 * selection also declares read-only, write-only, or read-write access so the Runtime runner can
 * validate inputs, invalidate stale output copies, and validate exact successful writes without
 * inspecting backend storage.
 *
