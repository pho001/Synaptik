 ...
 * cold geometry.
 * Schema 59 adds scalar BFLOAT16 pointwise bodies with raw represented locals and one rounding
 * boundary per producing instruction. Schema 60 applies only to pointwise intermediate
 * representations containing a cross-type CAST; same-type legacy pointwise projections retain
 * schema 52 or 59 class identity and unchanged generated bytes. Schema 61 adds bounded fixed-byte-
 * species publication and reload for dense FLOAT32/FLOAT64 mask boundaries while preserving
 ...
 * Schema 64 emits every proved FLOAT32/FLOAT64 vector scalar-power realization directly through
 * typed Vector API operations, removing the former generated dependency on scalar-power methods
 * in {@code CpuVectorMath}. The envelope version changes because generated bytes change; the
 * semantic realization and specialization identity fields do not. Schema 65 replaces the
 * impossible prepared CONCAT/STACK terminal exception path with the final statically prepared
 * occurrence arm and freezes typed native-order layouts at generation sites for affected
 * general-long segment loads and shared stores. These generated-byte changes preserve the
 * existing specialization identity projection. Schema 66 retains typed predefined unaligned
 * native-endian layouts directly for aggregate scalar segment entries, invalidating current-only
 * persistence envelopes because their generated bytes change while preserving every generated
...
public final class CpuGeneratorSchema {
     ...
     * FLOAT32-to-BFLOAT16 rounding boundary per logical producing instruction. Schema 60 adds
     * direct all-pair cross-type CAST bodies while preserving schema-52 and schema-59 projections
     ...
     * Every scalar convolution retains its historical schema-52 class projection. Schema 64
     * makes proved vector scalar-power bodies self-contained while retaining their existing
     * semantic realization and class-identity projection. Schema 65 replaces the impossible
     * prepared CONCAT/STACK terminal exception path with the final statically prepared occurrence
     * arm and freezes typed native-order layouts at generation sites for affected general-long
     * segment loads and shared stores, without changing the class-identity projection. Schema 66
     * retains typed predefined unaligned native-endian layouts directly in aggregate scalar
     * segment-entry setup, removing redundant native-order/layout recreation while preserving
