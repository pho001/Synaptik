 ...
 * remain cold facts.</p>
 *
 * <p>The ordinary aggregate representation records SUM, MEAN, PROD, MIN, MAX, ALL, or ANY; exact
 * ordinary attribute form; increasing selected-axis membership; retention; structural two-
 * boundary access; numerical state/domain shape; extrema floating policy; and complete-output-cell
 * ranges. Floating numerical rows use exact run-owned state, while all other rows are workspace-
 * free. Concrete layout magnitudes, carriers, slots, workers, and run identities remain cold.</p>
 *
