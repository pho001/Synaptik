...
sealed interface CpuBufferArgument {
    /** @return byte offset into a visible heap carrier, or zero for an exact segment argument */
    long byteOffset();

    /** @return exact selected region size in bytes */
    long byteSize();

    /** @return whether writes through the selected region are forbidden */
    boolean readOnly();
