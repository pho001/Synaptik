...
/**
 * CPU-private structural identity for one ordinary extrema or Boolean reduction.
 *
 ...
 * no partial, combine, or workspace state exists.</p>
 *
 ...
 * @param workspacePolicy {@link #ZERO_WORKSPACE}; forbids partial and combine state
 */
public record CpuAggregateIr(Kind kind, DataType dataType, Form form, int[] selectedAxes,
    ...
    /**
     * Identifies the four ordinary reduction semantics implemented by this CPU route.
     ...
     * @param workspacePolicy fixed zero-workspace policy
    ...
    public CpuAggregateIr {
        ...
        if (bool != (dataType == DataType.BOOL) || !bool && !numeric
                ...
                || rangePolicy != COMPLETE_OUTPUT_CELLS || workspacePolicy != ZERO_WORKSPACE)
