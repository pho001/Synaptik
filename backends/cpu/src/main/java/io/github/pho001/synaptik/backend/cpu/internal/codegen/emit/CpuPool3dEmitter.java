...
/**
 * Emits allocation-free scalar channels-first (NCDHW) max and fixed-divisor average Pool3d loops.
 *
 ...
 */
public final class CpuPool3dEmitter {
    ...
    /** Creates a stateless Pool3d Class-File emitter. */
    public CpuPool3dEmitter() {}
     ...
     * @param ir canonical Pool3d kernel identity
    ...
    public void emit(CodeBuilder code, CpuKernelSpecialization specialization, CpuKernelIr ir) {
        boolean max = ir.familyIdentity().startsWith("pool3d:kind=MAX:");
        boolean average = ir.familyIdentity().startsWith("pool3d:kind=AVERAGE:");
        ...
        if ((!max && !average)
            ...
            throw new IllegalArgumentException("Pool3d generated facts disagree");
        ...
        decode(code, geometry, 16, r, ow);
        decode(code, geometry, 15, r, oh);
        decode(code, geometry, 14, r, od);
        decode(code, geometry, 13, r, c);
        code.lload(r).lstore(n);
        code.lload(od);
        g(code, geometry, 25).lmul();
        g(code, geometry, 28).lsub().lstore(id);
        code.lload(oh);
        g(code, geometry, 26).lmul();
        g(code, geometry, 29).lsub().lstore(ih);
        code.lload(ow);
        g(code, geometry, 27).lmul();
        g(code, geometry, 30).lsub().lstore(iw);
        address(code, geometry, 1, 17, n, c, od, oh, ow, outAddress);
        if (max)
            emitMax(
                    code,
                    carriers,
    ...
    private static void emitMax(
        ...
        var ds = code.newLabel();
        var dd = code.newLabel();
        code.labelBinding(ds).lload(kd);
        g(code, geometry, 22).lcmp().branch(Opcode.IFGE, dd);
        code.lload(id0).lload(kd);
        g(code, geometry, 31).lmul().ladd().lstore(id);
        ...
        g(code, geometry, 23).lcmp().branch(Opcode.IFGE, hd);
        code.lload(ih0).lload(kh);
        g(code, geometry, 32).lmul().ladd().lstore(ih);
        code.loadConstant(0L).lstore(kw);
        var ws = code.newLabel();
        var wd = code.newLabel();
        code.labelBinding(ws).lload(kw);
        g(code, geometry, 24).lcmp().branch(Opcode.IFGE, wd);
        code.lload(iw0).lload(kw);
        g(code, geometry, 33).lmul().ladd().lstore(iw);
        var skip = code.newLabel();
        bounds(code, geometry, id, ih, iw, skip);
        address(code, geometry, 0, 7, n, c, id, ih, iw, address);
        ...
        loadValue(code, type, value);
        if (type == DataType.FLOAT64) code.dcmpl();
        else code.fcmpl();
        code.branch(Opcode.IFEQ, nonNan);
    ...
    private static void emitAverage(
        ...
        code.lload(iw0).lload(kw);
        g(code, geometry, 33).lmul().ladd().lstore(iw);
        bounds(code, geometry, id, ih, iw, padding);
        address(code, geometry, 0, 7, n, c, id, ih, iw, address);
        carriers.loadFrozen(type, s.carrierPattern().getFirst(), 0, address, false);
        storeRepresented(code, type, represented);
        decode(code, type, represented, value);
        ...
        code.lload(kd).loadConstant(1L).ladd().lstore(kd).branch(Opcode.GOTO, ds).labelBinding(dd);
        loadValue(code, type, sum);
        g(code, geometry, 34);
    ...
    private static void bounds(CodeBuilder c, int g, int d, int h, int w,
            java.lang.classfile.Label skip) {
        c.lload(d).loadConstant(0L).lcmp().branch(Opcode.IFLT, skip);
        ...
        c.lload(w);
        g(c, g, 6).lcmp().branch(Opcode.IFGE, skip);
    }

    private static void address(
            CodeBuilder c, int g, int base, int strides, int n, int ch, int d, int h, int w,
            int target) {
        g(c, g, base);
        c.lload(n);
        g(c, g, strides).lmul().ladd().lload(ch);
        g(c, g, strides + 1).lmul().ladd().lload(d);
        g(c, g, strides + 2).lmul().ladd().lload(h);
        g(c, g, strides + 3).lmul().ladd().lload(w);
        g(c, g, strides + 4).lmul().ladd().lstore(target);
    }
